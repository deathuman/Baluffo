...
def get_task_live_payload(
    ...
    *,
    ...
    grouped_history = ops_task_projection_mod.history_by_type(projection)
    if normalized_type == "fetch":
        return ops_task_fetch_live_mod.build_fetch_live_payload(
            context,
            projection=projection,
            task_state=task_state,
        )
    if normalized_type == "discovery":
        return ops_task_discovery_live_mod.build_discovery_live_payload(
            context,
            projection=projection,
            task_state=task_state,
        )
    if normalized_type == "sync":
        return ops_task_projection_mod.build_sync_live_payload(
            context,
            history_by_type=grouped_history,
        )
