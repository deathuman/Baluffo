...
def startup_banner(*, config: RuntimeConfig, bridge_log: Any) -> None:
    ...
    bridge_log(
        ...
        tasks="POST /tasks/run-jobs-bootstrap, POST /tasks/run-fetcher, POST /tasks/run-discovery, POST /tasks/run-sync-pull, POST /tasks/run-sync-push, POST /tasks/run-jobs-pipeline, GET/POST /tasks/jobs-pipeline-schedule, POST /tasks/job-availability-check, GET /tasks/job-availability-check-status, POST /tasks/abort, GET /tasks/run-jobs-pipeline-status",
    )
