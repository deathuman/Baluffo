    ...
    *,
    ...
    for source in registry_entries(registry_adapter):
        ...
        if entry_report["cacheDecision"] == "revalidate_only":
            ...
            # For structured sources we still re-fetch the current page when validators request it.
            _ = state_entry
                ...
                for listing_row in listing_rows:
                    ...
                    try:
                        detail_html = fetch_with_retries(
                            detail_url, fetch_text, timeout_s, retries, backoff_s
                        )
        ...
        entry_report["fetchedCount"] = discovered_candidates
        entry_report["keptCount"] = len(emitted_rows_for_source)
        if discovered_candidates > 0:
            entry_report["duplicateRate"] = round(
                max(0.0, discovered_candidates - len(emitted_rows_for_source))
                / float(discovered_candidates),
                4,
            )
        for row in emitted_rows_for_source:
