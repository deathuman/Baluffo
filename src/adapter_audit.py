...
AUDIT_CASES: list[dict[str, Any]] = [
    ...
    {"name": "8bitplay", "family": "community", "runner": community.run_8bitplay_source, "diagnostic": "8bitplay", "source_type": "built_in"},
    {"name": "gamesindustry", "family": "community", "runner": community.run_gamesindustry_source, "diagnostic": "gamesindustry", "source_type": "built_in"},
    {"name": "wellfound", "family": "community", "runner": community.run_wellfound_source, "diagnostic": "wellfound", "source_type": "built_in"},
    {"name": "epic_games_careers", "family": "community", "runner": community.run_epic_games_careers_source, "diagnostic": "epic_games_careers", "source_type": "built_in"},
    {"name": "greenhouse", "family": "provider", "runner": provider_api.run_greenhouse_boards_source, "diagnostic": "greenhouse_boards", "registry_adapter": "greenhouse"},
    {"name": "teamtailor", "family": "provider", "runner": provider_api.run_teamtailor_sources_source, "diagnostic": "teamtailor_sources", "registry_adapter": "teamtailor"},
    {"name": "lever", "family": "provider", "runner": provider_api.run_lever_sources_source, "diagnostic": "lever_sources", "registry_adapter": "lever"},
    {"name": "workable", "family": "provider", "runner": provider_api.run_workable_sources_source, "diagnostic": "workable_sources", "registry_adapter": "workable"},
    {"name": "smartrecruiters", "family": "provider", "runner": provider_api.run_smartrecruiters_sources_source, "diagnostic": "smartrecruiters_sources", "registry_adapter": "smartrecruiters"},
    {"name": "recruitee", "family": "provider", "runner": provider_api.run_recruitee_sources_source, "diagnostic": "recruitee_sources", "registry_adapter": "recruitee"},
    {"name": "pinpoint", "family": "provider", "runner": provider_api.run_pinpoint_sources_source, "diagnostic": "pinpoint_sources", "registry_adapter": "pinpoint"},
    {"name": "breezy", "family": "provider", "runner": provider_api.run_breezy_sources_source, "diagnostic": "breezy_sources", "registry_adapter": "breezy"},
    {"name": "jazzhr", "family": "provider", "runner": provider_api.run_jazzhr_sources_source, "diagnostic": "jazzhr_sources", "registry_adapter": "jazzhr"},
    {"name": "ashby", "family": "provider", "runner": provider_api.run_ashby_sources_source, "diagnostic": "ashby_sources", "registry_adapter": "ashby"},
    {"name": "personio", "family": "provider", "runner": provider_api.run_personio_sources_source, "diagnostic": "personio_sources", "registry_adapter": "personio"},
]
...
def _detail_examples(details: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for detail in details[:3]:
        rows.append(
            {
                "name": clean_text(detail.get("name")) or clean_text(detail.get("studio")) or "unknown",
...
def _bucket_for_result(*, jobs_count: int, error_text: str, details: list[dict[str, Any]], partial_errors: list[str]) -> str:
    ...
    if "parser_stale" in detail_classifications or "no jobs extracted" in combined_error:
        return "parser-stale"
    if any(token in detail_classifications for token in {"rate_limited", "dead_listing_page"}) or any(
        token in combined_error for token in ("429", "rate limit", "page not found", "not found", "forbidden")
    ):
        return "source-limited"
    if "no adapter plugin matched" in combined_error or "missing registry_adapter" in combined_error:
...
def _run_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    partial_errors = [clean_text(item) for item in (diagnostics.get("partialErrors") or []) if clean_text(item)]
    bucket = _bucket_for_result(jobs_count=jobs_count, error_text=error_text, details=details, partial_errors=partial_errors)
    result = {
        "adapter": case["name"],
        "family": case["family"],
        "bucket": bucket,
        ...
        "error": error_text,
        "sourceExamples": _registry_examples(case["registry_adapter"]) if case.get("registry_adapter") else [],
...
def _build_markdown(report: dict[str, Any]) -> str:
    ...
    for bucket in ("working", "mixed-success", "source-limited", "adapter-broken", "parser-stale", "follow-up-needed"):
        ...
        for row in items:
            ...
            ashby_summary = row.get("ashbySummary") if isinstance(row.get("ashbySummary"), dict) else None
            if ashby_summary:
                detail += (
