...
import {
  isFetcherBusy as isFetcherBusyFromModule,
  isDiscoveryBusy as isDiscoveryBusyFromModule,
  isOpsBusy as isOpsBusyFromModule,
  isSyncBusy as isSyncBusyFromModule,
  isPipelineBusy as isPipelineBusyFromModule,
...
import {
  isValidSourceFilter as isValidSourceFilterFromModule,
...
function isFetcherBusy() {
  return isFetcherBusyFromModule(adminBusyState);
}

function isDiscoveryBusy() {
  return isDiscoveryBusyFromModule(adminBusyState);
}

function isOpsBusy() {
  return isOpsBusyFromModule(adminBusyState);
}

function isSyncBusy() {
  return isSyncBusyFromModule(adminBusyState);
}

function isPipelineBusy() {
  return isPipelineBusyFromModule(adminBusyState);
}

function syncAdminBusyUi() {
  return syncAdminBusyUiFromModule({
    busyState: adminBusyState,
    viewState: toAdminViewState(),
    fetcherPresetMeta: FETCHER_PRESET_META,
    refs: {
      adminRunFetcherBtnEl,
      adminRunFetcherIncrementalBtnEl,
      adminRunFetcherForceBtnEl,
      adminRetryFailedBtnEl,
      adminRefreshReportBtnEl,
      adminRefreshBtnEl,
      adminRefreshOpsBtnEl,
      adminSyncTestBtnEl,
      adminSyncPullBtnEl,
      adminSyncPushBtnEl,
      adminSyncEnabledEl,
      adminRunDiscoveryBtnEl,
      adminLoadDiscoveryBtnEl,
      adminApproveSourcesBtnEl,
      adminRejectSourcesBtnEl,
      adminDeleteSourcesBtnEl,
      adminRestoreRejectedBtnEl,
      adminAddManualSourceBtnEl,
      adminManualSourceUrlEl,
      adminSourceFilterBtnEls,
      adminFetcherProgressBadgeEl,
      adminDiscoveryProgressBadgeEl,
      adminOpsProgressBadgeEl,
      adminContentEl
    },
...
function setDiscoveryLogOpen(nextOpen) {
  return setDiscoveryLogOpenFromModule(adminDiscoveryLogDetailsEl, nextOpen, {
    onSyncStart: () => {
...
function syncDiscoveryLogDisclosure() {
  return syncDiscoveryLogDisclosureFromModule(adminDiscoveryLogDetailsEl, {
...
function setBusyFlag(key, value) {
  setBusyFlagFromModule(adminBusyState, key, value);
  syncAdminBusyUi();
}

function resetBusyFlags() {
  resetBusyFlagsFromModule(adminBusyState);
  syncAdminBusyUi();
}
...
function cacheDom() {
  ({
    ...
    adminRejectSourcesBtnEl,
    adminDeleteSourcesBtnEl,
    adminDiscoverySummaryEl,
    adminManualSourceUrlEl,
    adminAddManualSourceBtnEl,
    adminManualSourceFeedbackEl,
    adminPendingSourcesEl,
    adminActiveSourcesEl,
    adminRejectedSourcesEl,
    adminRestoreRejectedBtnEl,
    adminDiscoveryLogEl,
    adminDiscoveryLogDetailsEl,
    adminBridgeStatusBadgeEl,
    adminShowZeroJobsToggleEl,
    adminRefreshOpsBtnEl,
    adminSyncPullBtnEl,
    adminSyncPushBtnEl,
    adminSyncTestBtnEl,
    adminSyncStatusEl,
    adminSyncEnabledEl,
    adminSyncConfigHintEl,
    adminOpsAlertsEl,
    adminOpsKpisEl,
    adminOpsScheduleEl,
    adminOpsFetcherMetricsEl,
    adminOpsTrendsEl,
    adminOpsHistoryEl,
    adminFetcherProgressBadgeEl,
    adminDiscoveryProgressBadgeEl,
...
function isValidSourceFilter(value) {
  return isValidSourceFilterFromModule(value);
}

function normalizeSourceFilter(value) {
  return normalizeSourceFilterFromModule(value);
}

function setSourceFilter(value) {
  activeSourceFilter = setSourceFilterValue(value, {
    normalizeSourceFilter,
    writeSourceFilter,
    sourceFilterKey: ADMIN_SOURCE_FILTER_KEY,
    buttons: adminSourceFilterBtnEls
  });
}
...
function toAdminViewState() {
  return toAdminViewStateFromModule(adminBusyState, {
