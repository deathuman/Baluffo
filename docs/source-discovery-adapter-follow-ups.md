...
- `gamedevmap_active_dry_run.py::_queue_no_careers_recovery` and `_apply_recovery_results` still keep active-audit recovery grouping, wave finalization, and rejection semantics local. Extract these only through a strategy contract that preserves GameDevMap's artifact shape.
