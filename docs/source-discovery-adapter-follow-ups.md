...
- Generalize the resumable audit engine for sheet-directory and web-search sources where a full scan has meaningful intermediate progress, timing, cache, and resume needs.
...
- Use the new directory audit report evidence to decide whether Gamesmap or Gameprog audits should become default in a later explicit plan.
...
- Gamesmap and Gameprog now share cache, fetch-job, and opt-in audit-engine seams. Future work can decide whether either audit path should become default after live runtime and artifact-size evidence.
