...
`staticStillActiveDespiteValidatedProvider` is a suppression-readiness diagnostic. It means the provider/static pair has validated provider evidence while the linked static row is still active or not currently emitting dynamic suppression evidence. It does not authorize static deletion, tombstoning, rejection, permanent suppression, or source-registry cleanup.
...
| Only ambiguous candidates | Multiple static rows match the same provider and no deterministic evidence selects one. | Review `ambiguityGroups`, `candidateStatics`, ignored alternatives, and `actionableBlockedCandidates`. Provider-shaped self-links are diagnostic only; do not apply a link until one candidate becomes `apiEligible=true` with clear evidence. |
...
Dedup auditability is the product-risk gate before lifecycle UX; [`archive/read-only-lifecycle-ux-closeout.md`](archive/read-only-lifecycle-ux-closeout.md) records the first read-only label/filter slice after provider/static reconciliation closed. Start with `dedupAuditGate.blockerDetails` in Admin/Ops Dedup Lists: each blocking issue gives the count, cause breakdown, reason it blocks lifecycle UX, next action, and capped examples. Use supporting diagnostics only after that first pass: current-run `mergedCount`, `mergeReasonCounts`, `currentRunMergeExamplesByReason`, current-vs-carried `sourceBundleCollisionCount`, origin-split review queue cause counts, suspected causes, identity quality, non-provider provenance, Google Sheets role-bucket diagnostics, provider/static disagreement examples/classifications, `providerStaticDisagreementGateCounts`, dedicated provider/static title-company collision examples, and `dedupAuditGate.warningDetails`. Do not expand lifecycle UX beyond the conservative first slice unless `dedupAuditGate.lifecycleUxReady=true` remains true in real-data evidence, current-run merges are explainable, provider/static disagreement is low or reviewed with URL/source evidence, Google Sheets generic role/category merges are blocked for new runs, and carried source-bundle collisions are clearly historical, reconciled, or low-risk.
...
Ops health and Admin reporting also expose a read-only dedup review-state status block with the artifact path, any missing/malformed read warning, the reviewed pair count, the `reviewed_safe` count, the `confirmed_blocking` count, and the remaining unresolved blocking count so operators can tell when the blocker is really unresolved versus just unreadable local state. If the provider/static blocker detail reports current-run blockers, restoring an older `dedup-review-state.json` cannot clear those current-run rows; fix or re-review the current evidence. If the blocker detail reports carried blockers, restoring or recreating reviewed-safe local evidence can downgrade those exact carried rows to warnings.
