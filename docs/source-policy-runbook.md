...
If the provider is `ready_later` with enough consecutive successes but suppression remains `0`, check `sections.suppressionEligibility`. A row with `reason="linked_static_not_selected"` means the provider is ready but the linked static source was not part of the selected/default source set for that fetch, so no runtime suppression row could be emitted.
...
| Provider ready but suppression count is `0` | The provider has enough successful fetches, but the linked static source was not selected or is missing from active/pending registry rows. | Check `sections.suppressionEligibility.missingLinkedStaticRows`. `linked_static_not_selected` is visibility-only; do not force-select or mutate registry rows just to create a suppression row. |
