...
- **The Jobs page update caption now speaks plain language instead of pipeline vocabulary.** The caption under `Update jobs` was reusing the same formatter as the Admin diagnostics view, so an end user could see a raw adapter id and internal counters — the worst case read `Probing candidates · stage 2/5 · probing steam_curator_feeds · 138/500 candidates probed · generated 312 · endpoints 87 · queued 41`. It is now capped at two plain segments (`138 of 500 sources · ETA 4m`), and the phase name is deliberately omitted when a count is shown because the button label directly above already names the stage. Adapter ids, `stage`/`step` markers, `rate N/min`, and the `generated`/`endpoints`/`queued`/`batch`/`URLs`/`pages` counters are filtered out; an unrecognised internal token falls back to the user-facing stage vocabulary rather than being prettified into view (`steam_curator_feeds` must not render as "Steam Curator Feeds"). Nothing is lost: the full technical string still rides in the button tooltip while a run is active, and Admin remains the canonical diagnostics surface — the same user-facing-stage copy decision the desktop UX polish pass recorded. The stalled-status caption also drops "Pipeline status delayed; retrying..." for "Still working…". `formatBlockingTaskProgressLabel` is unchanged, so Admin output is byte-for-byte identical.
