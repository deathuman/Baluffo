...
- **The registry now has a standing hygiene monitor plus two new hard structural guards.** The existing runtime report only flagged asset pages, and every structural guard read the *committed seed* while the registry the pipeline actually fetches had drifted from it — 2,245 live rows against 1,893 in the seed, with 31 duplicate-URL groups present only in the live view. A new advisory `registryHygieneAudit` block (alongside the unchanged `registryAssetPageAudit`) reports bounded evidence for asset pages, canonical-URL duplicate candidates, host drift, and repeated unreachable source state, with exact counts over capped 20-source / 3-sample lists. It is read-only: it never demotes, deletes, or rewrites a row, and a nonzero count is a review signal, not a failure. Duplicates split into `knownCollisionGroupCount` and `uncoveredDuplicateGroupCount` using the existing reviewed-collision baseline, so genuinely new drift is separable from grandfathered twins; an unloadable baseline reports everything as uncovered so drift is never masked. Unreachable evidence requires at least two consecutive failures plus permanent 404/410, not-found, DNS/TLS, or connection errors, so a single transient error never flags, and provider rows are matched to source state by registry name rather than id so reachability is attributed to the row that actually failed. The `registry` guardrail group gained two deterministic checks with no allowlist: one registry id claimed by several rows (the id keys source state, tombstones, and suppression evidence, so twins make every one of those lookups ambiguous), and any page entry that is not an absolute http(s) URL with a host (generalizing the existing `data:`-only inline-asset rule to relative, `blob:`, `javascript:`, empty, and hostless entries). Both are green on the active seed, the pending seed, and the live registry.

- **Dead-domain repair candidates now need repeated evidence, and the first cut of that rule was wrong.** The registry-hygiene monitor flagged unreachable sources off a single `consecutiveFailures >= 2` snapshot, which measures "this run failed twice", not "this domain is dead". The promotion rule to a repair candidate initially required `failures >= 3` **and** `outageDays >= 7`, and checking that against the live source state showed it missed the worst rows on purpose: the four genuinely dead boards had 125–167 day outages but only **two** recorded failures each, because a circuit-broken or cadence-skipped source simply stops being retried and the counter stops incrementing. A conjunction therefore under-reports exactly the dead domains the signal exists to find. Repetition is now accepted in either form — `failures >= 3` **or** `outageDays >= 7` — with a third condition, `observationAgeDays <= 30`, so the promotion cannot fire on ancient bookkeeping for a source nobody has re-checked. A row that never succeeded has no outage anchor and reports `outageDays: 0` rather than a fabricated span. Each promoted row now carries `consecutiveFailures`, `outageDays`, `observationAgeDays`, the classified `unreachableEvidence`, and a 200-character `lastErrorSample` so a reviewer does not have to re-run the source to judge it. The error classifier was checked against all 478 error-status rows in the live state rather than assumed: it separates 31 DNS/TLS and 6 HTTP 404 rows from the 441 transient ones, and correctly leaves `403` bot-blocks, timeouts, and the 193 `no jobs extracted` rows alone, since those are not dead domains. **Current live answer: zero repair candidates** — all four dead sources found in source state belong to registry rows that earlier repointing and pruning waves already removed, so the monitor declines to manufacture work for a board that no longer exists. Still advisory throughout: promotion means a row is ready for a human-approved repair decision, never an automatic mutation.

- **Registry repairs now have a human-approval gate, and the gate records decisions without ever applying them.** `data/registry-repair-review.json` captures what a human decided about each registry-hygiene finding — `acknowledged`, `repair_approved`, or `snoozed`, keyed per `(sourceId, findingKind)` so approving a dead-domain repair says nothing about a duplicate-URL collision on the same row. This is the approval half only: reading it annotates the advisory report so an already-adjudicated row stops re-reporting as fresh work, and nothing in the artifact or the audit writes, demotes, deletes, repoints, or tombstones a registry row. Applying an approved repair stays a separate operator action through the existing sanctioned registry transition paths, so no new mutation path is introduced. The property that makes the gate more than bookkeeping is the **evidence fingerprint**: a decision is bound to what the defect *is* — source id, finding kind, registry state, classified failure mode, affected URLs — and is honored only while that still matches. A row repointed elsewhere or now failing with a different error class reports `stale` and visibly returns to the operator's queue instead of inheriting an approval made about a different defect, and a decision recorded with no fingerprint never matches because it cannot be traced to anything. The fingerprint deliberately **excludes** the climbing counters (`consecutiveFailures`, `outageDays`, `observationAgeDays`): folding those in would void a standing review after one more run and train the operator to re-approve forever, which is worse than having no review at all. A missing or malformed artifact degrades to "nothing reviewed" plus a warning code, never an exception, so a corrupt gate cannot look like an approval.

- Fixed the pre-existing Admin idle health polling error `loadOpsHealthData is not a function`: the health coordinator now passes its existing refresh callback to the polling loader. A real-controller timer regression test reproduces the failure when that wiring is removed and verifies refresh completion with it restored. This also restores the callback used by idle recovery.

- Admin follow-up polish makes source ID icons 16px, full-opacity and theme-aware, removes their redundant outer ring, and preserves tooltips with keyboard-focusable targets. Advanced bulk actions suppresses native disclosure markers while retaining its authored indicator; Run trends gains separation from the following tabs. Task-progress helper text now matches the 0.82rem secondary-note scale. Fixed source-row heights and table columns are unchanged. These changes have source/render regression coverage; fresh light/dark responsive browser verification is still pending.

- **The Admin Action Center was polling a route that structurally could not produce two of its four signals.** `frontend/admin/app/action-center.js` read `/ops/health?view=ready`, whose payload is built by `compute_ops_health_ready()` (`src/bridge/ops_api_health.py`) and carries **neither `alerts` nor `kpis`**. `evaluateStaleFetch` and `evaluateFailedSources` both read those keys, so the stale-fetch and failed-source signals were dead code and only `storage_health` / `sync_status` could ever render. **Every unit test still passed**, because they injected synthetic payloads keyed on whatever path the controller happened to request — the assertion that would have caught it is that the *route* carries the fields, not that the renderer can draw them, and a regression test now asserts exactly that by driving the controller from a real-route-shaped payload. The route is now `/ops/fetch-kpis?view=summary`, chosen by measuring the candidates rather than by preference: it returns both `alerts` and `kpis` in **37ms** behind a 15s TTL cache. `/ops/dashboard-health?view=summary` also has `alerts` but only three KPI keys and omits `lastSuccessfulFetchAge`, which rendered the literal string **"Last successful fetch was at an unknown time"** — that missing-age path is now guarded in `formatAge`, since `parseAge` returns `Infinity` for an absent age and the string `Infinityd` was reachable from any route with an empty `kpis`. The full `/ops/dashboard-health` route was rejected: it has both fields but is uncached and records 0.5–1.6s per chain on an idle Pi against a 0.75s gateway cutoff. The backend was deliberately left alone — `tests/bridge/test_lightweight_startup_routes.py` pins that the ready view must not build full ops health. Because the new route is one the startup lane serializes, the Action Center's first poll is now enqueued on that lane (injected as `enqueueStartupTask`), which is what keeps `STARTUP_HEAVY_ROUTES` from overlapping at boot; the injection defaults to an immediate call, so unit fixtures are unaffected.
- **The Action Center and the Ops Health panel no longer both report the same fetch alert.** `stale_fetch` and `fetch_never_run` are Action Center-owned, so `renderAdminOpsAlerts` filters them out and the Operations panel shows only Ops-specific alerts. The filter is render-time only, so the cached payload the Ops KPI controller asserts on is untouched, and the empty state collapses (`min-height: 0`) instead of leaving a blank box.
- **The Admin source table shows the URL where it used to repeat the Studio.** Studio already appears verbatim inside the Name on **524 of 525 rows**, so the column restated its neighbour; the URL is unique on **250 of 250** rows and is the field that actually distinguishes two same-named sources — the live payload has `Clay Token Game Studio (GameDevMap)` five times with URLs `/about/careers`, `/careers`, `/company/careers`, `/jobs` and `/join-us`. The plan that prompted this claimed the Adapter, Status and Approval columns were uniform and could be dropped; that was **wrong**, and the measurement that disproved it is worth recording: uniformity was an artefact of the **39-row virtual window**, and over the full filtered bucket the pending set carries 8 distinct adapters, active 13 and rejected 6. No column was dropped. The URL column is deliberately capped at 1437px in the mobile track list rather than sized to its longest value.
- The Admin page's `i` info glyph is now an inline `fill="currentColor"` SVG, matching the icon convention the rest of the page already used. It reuses `ACTION_CENTER_ICONS.info` from `frontend/admin/render/action-center.js`, so `render/sources.js` gained a leaf-to-leaf import; the affordance and its tooltip are unchanged.
- **Empty Admin log panels no longer reserve height they are not using.** The fetch and discovery logs held a 210px and 200px reservoir while showing a single placeholder line, which is most of the time. A `:has(> .admin-fetcher-line:only-child)` rule collapses the box to its content, and the reservoir is restored the moment a second line arrives, so the populated case does not jump. Measured on the live page: `#admin-fetcher-log` 210px → 49px, `#admin-discovery-log` 200px → 48px, page height 4856px → 4542px at 1440px.
- **The Admin discovery action buttons size to their labels.** `flex: 1 1 140px` made "Approve Selected" 527px wide to display 125px of text, and the equal-thirds filter grid made the 19px "All" chip 349px. Worst-case wasted width fell from **420px to 31px** across 520/900/1440px, with no horizontal overflow at any of them.
- **The Admin heading outline read backwards.** `.admin-section-title` is worn by both the H2 section headings and the H4 sub-headings and carried one flat `1.05rem`, so measured on the live page H2 was 16.8px, H3 16px and H4 **16.8px** — the H4s nested inside an H3 section rendered larger than their own parent. Sizes now come from the element (H2 1.2rem, H3 1.1rem, H4 1rem), giving 0 inversions; the two context overrides stay margin-only so they cannot flatten the scale again.
- **An Ops tab badge could spin forever on a count that would never arrive.** Both "still fetching" and "will never arrive" arrived as `loaded: false`, and only the presence of an `error` code separates them — the backend already sends `dedup_evidence_missing` for dedup, and the frontend was discarding that distinction. Badges now render `...` while pending and `-` once failed, so the live dedup badge resolves immediately instead of staying on `...` until its tab is clicked. The backend contract is unchanged; two Python tests pin `loaded is False` for that route.

- **The Admin Action Center no longer renders as a placeholder.** It was a 1152×188px card holding one sentence and one full-size primary button, and all five states (`checking` / `partial` / `healthy` / `unavailable` / `active-work-delayed`) rendered through a single function, so **"All systems operational" and "Operational signals unavailable" produced pixel-identical markup** apart from the text — a passing health check was indistinguishable from a failed one. Its glyphs were emoji and ASCII (`i`, `⚠`, `⏰`, `↔️`, `❌`, `✓`, `▶`, `🔄`, `📋`, `✕`) where the two icon buttons elsewhere on the page use monochrome inline SVG. Presentation now lives in a leaf renderer, `frontend/admin/render/action-center.js`, which owns a per-state tone/icon/chip table: the healthy case collapses to one compact line with a green accent instead of a full-width box, each state carries its own left accent and status chip, the panel header follows the page's existing `.admin-fetcher-header` convention (title left, actions right) with the status chip and a `checked Ns ago` stamp, "Copy all diagnostics" is demoted to a 2rem icon button reusing `.admin-fetcher-icon-btn`, and every glyph is a `fill="currentColor"` SVG path. Signal rows are now a three-column grid — severity chip, message, right-aligned actions — stacking below 520px so a long message cannot squeeze the actions out of the card. Verified on the live page: all five states render distinctly, zero overflow at 520px through 1600px, in both themes.
- **A "View all → Ops Health" overflow row in the Action Center was unreachable and has been removed.** It was not a comparison mistake: `stale_fetch` fires only when the fetch age exceeds 12h and `failed_sources` only when it is at or below 12h, so those two signals are **mutually exclusive by construction** and the signal space tops out at exactly three — the same as `MAX_ITEMS`. A 192-combination sweep of the live signal space confirmed three is the maximum. The cap is retained as a display contract, and a test now pins the mutual exclusion so that adding a fifth signal fails loudly instead of silently reintroducing an invisible affordance. The duplicated unreachable `!pollMeta.anyChecked` / fallback branches that both rendered the unavailable state were collapsed into one.
- The Action Center's storage signal no longer reads `Storage health issue detected — Storage is unhealthy`, which restated its own headline; the summary is now specific (`SQLite integrity check failed`, or the diagnostic failure count). A text-similarity filter that hid such restatements was tried and rejected — it scored the genuinely useful sync summary ("Sync needs attention" / "Sync conflict needs review; data refresh can continue") as a duplicate and suppressed it.
- **Pre-existing, not introduced here:** `loadOpsHealthData is not a function` is thrown once per Admin page load from `frontend/admin/app/ops/health-loaders.js:102`. Reproduced identically on a pristine `HEAD` worktree, so it predates this change and is left alone. It does not affect the Action Center, which renders correctly on the same load.

- **The Admin operations run table no longer clamps its own columns, and a run's detail moved into the inspector drawer.** Every cell in the runs table was ellipsized, and the tracks were sized from guesses rather than from what they had to hold. Measured on the live page with real rows, three columns were clipping: **Status reserved 88px for a `completed with warnings` chip that needs 160px**, **Finished reserved 104px for `21 Sept 2019 12:12` that needs 114px** (the compact date gained a year suffix for non-current years, which the old 144px track also could not fit — the original complaint), and **Progress needed 335px against a 320px track at every width from 521px to 1600px**. Two columns were also reserving width they never used: Duration held 64px for a 13px `0s` and Failed 64px for an 8px `0`. All seven tracks are now sized from the measured natural width of the widest value each column can render — `4.25rem 10.5rem 4.25rem minmax(21.5rem, 1fr) 3.5rem 7.5rem 4.5rem`, with a `0.5rem` gap that is what stops the right-aligned `FINISHED` header reading as one word with the left-aligned `ACTIONS`. **Result: zero clipped cells at every width from 521px to 1600px**, where before there were three. The previous fix attempt — keeping `width: max-content` and capping the `fr` column at `20rem` — was measured and rejected: it made header and rows disagree (848px against 850px at a 1000px viewport) *and* kept the progress cell ellipsized at every width, because the cap that bought alignment also clamped the cell. The wrappers now take `width: 100%` with a `min-width: 59rem` floor, so both grids resolve against one definite width and the `fr` track absorbs the card's slack instead of leaving it unallocated on the right; below the floor the wrapper scrolls rather than squeezing tracks below what their content needs. The floor is wider than the old 872px of fixed tracks precisely because two columns had been lying about their size. Alongside that, **the inline "Selected Run Analysis" panel is gone**: clicking a run now opens the existing right-side inspector drawer, which renders a reorganized detail view from a new pure renderer (`frontend/admin/render/ops-run-detail.js`) — identity and status, a facts strip, a labeled stat grid, slowest sources, work items, a de-duplicated timeline and actionable diagnostic hints, with empty sections omitted entirely rather than shown blank. Two pre-existing defects in that drawer were found and fixed while wiring it: `admin.html` ships the overlay with a `hidden` class that `components.css` makes `!important` and nothing ever removed, so **the drawer had never opened at all**; and the inspector's click delegate rejected any target inside a `<details>`, which made every row in the "Recent Runs" and "Older runs" disclosures unreachable. A latent narrow-layout bug went with them — the `520px` block hid the Finished cell but left the Actions cell in the DOM, wrapping it onto an implicit second grid row that the 52px `overflow: hidden` then clipped, so the Abort button rendered but could not be seen or clicked; both trailing cells are now hidden together, and Abort stays reachable from the drawer. One more defect surfaced only under the scroll check: the "Current Runs" / "Recent Runs (n)" / "Older runs (n)" labels are children of the `overflow-x: auto` wrapper, so scrolling right carried them out of view — pre-existing at HEAD (measured: 138px of scroll at a 900px viewport), but the wider tracks made it worse (198px), leaving the row reading as a bare `2) -`. The labels are now `position: sticky; left: 0`, which pins the text to the scroller's left edge while the box keeps its full width so the background band still spans the table.
...
- **New npm advisories now surface on PRs and main pushes instead of only on release-day Dependabot rescans: `lint.yml` runs a `security:js` lane (`npm audit --json` over the committed lockfile, severity floor `moderate`) beside the Python pip-audit lane, enforcement lives in `scripts/js_security_audit.py` (fails closed on missing npm/unparsable reports, allowlist-suppressed advisories must still be explicitly accepted via the expiry-enforced `tools/security/npm-audit-allowlist.json`), `.github/dependabot.yml` registers the npm ecosystem so PR manifests get Dependabot's alert preview and weekly grouped dev-tooling update PRs, and the workflow guardrail contract pins all three so the lanes cannot silently vanish.** Motivated by the three advisories (js-yaml, smol-toml, @humanfs) that sat on `main` until release push; `npm audit` currently reads zero findings. Tests: 12 new in `tests/test_js_security_audit.py` (severity floor, allowlist + partially-allowlisted chains, transitive-effect records, fix-availability rendering, fail-closed paths, committed-inputs pin); full suite green, changed-mode gate exit 0.
...
- **CodeQL's six open alerts are resolved at the source: the script/style strip regexes now accept loose end tags (`</script >`), the report-history slot filename component is derived from a runId digest (structurally incapable of path traversal), and the four test-side incomplete-URL-substring findings now assert on parsed hostnames or full canonical id sets (2026-09-16, CodeQL alerts #119–#129).** The `py/bad-tag-filter` fix (`_LIST_ONLY_SCRIPT_STYLE_RE`, plus the same-class siblings `_NON_VISIBLE_BLOCK_RE` in the no-openings detector and the discovery probe's block stripper) closes the `</script >`/`</style >` gap where scripted heading text could survive stripping and leak into the row universe, pinned by a new loose-end-tag case; `py/path-injection` ends the class outright — the history-slot filename component is a 16-hex digest of the runId rather than slug text, so a hostile runId can never shape or escape the path while the payload runId stays verbatim and dedup keys on payload content; the test rewrites replace substring assertions with hostname-set/full-id comparisons so lookalike hosts or ids cannot satisfy them. Bonus hardening surfaced by the new tie test: the slot-writer uniquify separator changed `-` → `_` so an mtime-tied same-run dedup can no longer keep the stale payload over the fresh one (deterministically pinned by forcing equal mtimes). Full suite 5,254 passed / 2 skipped; mypy clean; changed-mode gate exit 0.
