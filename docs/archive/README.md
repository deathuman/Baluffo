...
- Completed static plugin simple runner migration lives in [`static-plugin-simple-runner-migration-plan.md`](static-plugin-simple-runner-migration-plan.md). 5 plugins converted to SimpleStaticPlugin (remedy, supercell, larian, activision, riot), 1 shared helper extracted (`generic_parser_then_detail_links`), 10 plugins left custom with documented rationale, 2 dead functions removed from `_runner.py`. All 183 tests pass. 14 loopholes closed during audit.
- Superseded `/tasks/abort` non-blocking + telemetry precursor plan lives in [`PLAN-abort-telemetry.md`](PLAN-abort-telemetry.md); it is superseded by the implemented baseline in [`task-abort-control-plan.md`](../plans/task-abort-control-plan.md).
