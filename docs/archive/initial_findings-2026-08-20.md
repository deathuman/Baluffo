# Initial Refactoring Findings — Archived 2026-08-20

> - **Status:** Archived — historical record only (was Active plan, advisory-only)
> - **Use this when:** checking provenance for the 2026-05-17 analysis or the 16 splits closed through 2026-08-20 (`reporting_dedup_evidence` 1,133→12 + `state_lifecycle` 1,121→38 now done)
> - **Canonical for:** the 2026-05-17 initial refactoring target inventory as archived; not canonical for current routing
> - **Not canonical for:** current runtime contracts, payload shapes, source registry policy, bridge route contracts, or implementation state after any later refactor lands — use [`../architecture-ai-map.md`](../architecture-ai-map.md) and [`../AI_ASSISTANT_GUIDE.md`](../AI_ASSISTANT_GUIDE.md) instead
> - **Then inspect:** [`../AI_ASSISTANT_GUIDE.md`](../AI_ASSISTANT_GUIDE.md), [`../architecture-ai-map.md`](../architecture-ai-map.md), [`../plans/refactor-charter-template.md`](../plans/refactor-charter-template.md), and [`../testing.md`](../testing.md)
> - **Last updated:** 2026-08-20 — archived after `reporting_dedup_evidence 1,133→12` (12/12) + `state_lifecycle 1,121→38` (45/49 + 4 seam diffs via `_sl.`) splits + §3F/snapshot/Findings refresh; when-to-split now **13 files → 0 candidates** (only deferred `sync_service` 1,019 remains ≥1k in an active subsystem). (Earlier 2026-08-20: `reporting_dedup` split + §3F re-verify; 2026-08-19: hotspot refresh + items 5–7,10,13,15 done + §10 rule.)
> - **Location:** `docs/archive/initial_findings-2026-08-20.md` (archived from `docs/plans/initial_findings.md`)

> **Archived 2026-08-20:** 16 splits now done (including the final `reporting_dedup_evidence` and `state_lifecycle` pair). For current file routing and hotspot ranking, use [`architecture-ai-map.md`](../architecture-ai-map.md) and run `python tools/repo_health/bin/analyze_refactorability.py` — do not treat this inventory as the current hotspot list.
...
- Split `src/jobs/reporting_dedup_evidence.py` behind its existing public builders. **Done** (2026-08-20 — `1,133 → 12`-line coordinator + 3 leaves `dedup_evidence_review_queue.py:285` + `dedup_evidence_audit_gate_payload.py:302` + `dedup_evidence_bundle_report.py:596`, **12/12 byte-identical**; was 3,641 at analysis time).

**Current (2026-08-20) top remaining targets** — see §3F for the full refreshed ranking: `src/source_discovery/gamedevmap_active_dry_run.py` (2,087) is now the largest file in `src/`; **no jobs-core hotspot remains — 0 files exceed 1,000 lines in `src/jobs` core** — largest are `src/jobs/state_source_records.py:864` and `src/jobs/pipeline_source_results.py:807` (`reporting_dedup_evidence.py` `1,133 → 12` and `state_lifecycle.py` `1,121 → 38` splits 2026-08-20 are now done; both now 12/38-line coordinators, no longer hotspots; `dedup.py`, `canonicalize_google_sheets.py`, `static_listing.py`, `source_registry_io.py`, and `static_detail_heuristics.py` were split 2026-08-19 and are no longer hotspots).
...
**Refreshed 2026-08-20** against the now-committed tree (current `wc -l`; direct `.py` files per directory — nested subdirectories excluded, so the four `src/jobs` rows stay disjoint).

## Codebase Snapshot

| Area | Files | Δ vs 2026-05-17 | Large Files (>500 lines) |
|------|-------|-----------------|--------------------------|
| `src/jobs/` (core) | 70 | +29 | 13 — top: `pipeline_source_results.py:807` |
| `src/jobs/common/` | 41 | +10 | 8 — top: `dedup_evidence_bundle.py:798` (+ `dedup_evidence_bundle_report.py:596`) |
...
Large-file counts moved as the coordinator+leaves splits landed: `dedup.py` (1,804→286), `canonicalize_google_sheets.py` (1,607→45), `reporting_dedup_evidence.py` (3,641→1,133→12) and `state_lifecycle.py` (1,121→38) dropped out of `src/jobs` core (former 2026-08-20 into `dedup_evidence_review_queue:285` + `audit_gate_payload:302` + `bundle_report:596`; latter 2026-08-20 into `state_lifecycle_{identity:120,availability:236,normalization:257,transitions:291,orchestration:321}`), while `src/bridge` grew by the `task_launch_api.py` (2,441) and `pipeline_service.py` split leaves. See §3F for the repo-wide hotspot ranking this snapshot now matches.
...
**Refreshed 2026-08-20** (current `wc -l`, post-commit):
...
| `src/source_registry_io.py` | ~n/a | **149** (coordinator; split 2026-08-19 into paths/load/journal/save leaves — all < 500: `load` 465, `journal` 426, `paths` 330, `save` 229) |
...
| `src/jobs/reporting_dedup_evidence.py` | 3,641 | **12** (coordinator; split 2026-08-20 into `dedup_evidence_review_queue.py:285`, `dedup_evidence_audit_gate_payload.py:302`, `dedup_evidence_bundle_report.py:596` — no longer a hotspot) |
| `src/jobs/state_lifecycle.py` | ~n/a | **38** (coordinator; split 2026-08-20 into `state_lifecycle_{identity:120,availability:236,normalization:257,transitions:291,orchestration:321}.py` — all <500, 45/49 byte-identical + 4 seam diffs) |
| `src/jobs/pipeline_source_results.py` | 721 | **807** |
| `src/jobs/canonicalize.py` | 831 | **150** (thin coordinator post-split) |

**0 files now exceed 1,000 lines in `src/jobs` core.** `dedup.py`, `canonicalize_google_sheets.py`, `static_listing.py`, `reporting_dedup_evidence.py`, and `state_lifecycle.py` dropped out after their coordinators landed (`static_listing.py` 52, `reporting_dedup_evidence.py` 12, `state_lifecycle.py` 38). The largest files under `src/jobs` are now `state_source_records.py:864` and `pipeline_source_results.py:807` (and `pipeline_run_setup.py:712`); the largest `src/jobs/adapters/` files are `provider_structured_listing.py` (860) and `location_rules.py` (856) — see §3F.
...
### 3A. `reporting_dedup_evidence.py` (3,641 lines at analysis time — **12 today** — split 2026-08-20 into `dedup_evidence_review_queue:285` + `dedup_evidence_audit_gate_payload:302` + `dedup_evidence_bundle_report:596`, item 11 done)
...
### 3F. Current Hotspot Ranking (refreshed 2026-08-20)

From the current `analyze_refactorability.py` run (re-run 2026-08-20 against the committed tree — after `reporting_dedup 1,133→12` and `state_lifecycle 1,121→38` splits): **108 files ≥ 500 lines** (down from 109). The tool's top-10 is now **size-primary** (line count descending, score only a tiebreak — changed 2026-08-19), so the largest pure-size files surface instead of being hidden behind runtime-named files. The table below is the tool's exact top-10 output; scoring is `size: 25/50` + `runtime-hotspot: 30` (name-match on runtime/app/domain/orchestrator/bridge/pipeline/main).

**Tool top-10 (size-primary, after both 2026-08-20 splits):**
...
| 9 | `src/ship/runtime_launcher.py` | 1,103 | 80 |
| 10 | `src/container_gateway.py` | 1,084 | 50 |
...
**Score 50 — high-risk size only (15 files, after both 2026-08-20 splits):** the pure-size tier. These are the actual largest files in the repo and the natural next split targets (`reporting_dedup_evidence` 12 + `state_lifecycle` 38 are now coordinators; largest leaves `dedup_evidence_bundle_report:596` and `state_lifecycle_orchestration:321` are below this tier):
...
| `src/bridge/sync_service.py` | 1,019 | Bridge — largest active bridge file, **deferred** per §10 |
...
**Jobs-side takeaway**: `dedup.py` (1,804), `canonicalize_google_sheets.py` (1,607), `static_listing.py` (2,237), `static_detail_heuristics.py` (1,153) were split 2026-08-19; `reporting_dedup_evidence.py` (1,133 → 12) and `state_lifecycle.py` (1,121 → 38) were split 2026-08-20 into coordinators + leaves and are no longer hotspots; **0 files now exceed 1,000 lines in `src/jobs` core** — see snapshot. The largest `src/jobs` file is now `pipeline_source_results.py:807` (and `pipeline_run_setup.py:713`). No jobs-core file is a split candidate — see verdict below.
...
**When-to-split verdict (2026-08-20, §10 rule applied; refreshed after both 2026-08-20 splits).** Of the **13 files ≥ 1,000 lines** in `src/` (down from 15 after `reporting_dedup 1,133→12` and `state_lifecycle 1,121→38`), **0 files meet all three bars**. The only remaining ≥1,000 file in an active subsystem is `sync_service.py` (1,019, bridge) which is **formally deferred** per seam audit (holder singleton + 6 Protocols + `RLock` + `BridgeApi` wiring), not a split candidate. `reporting_dedup` and `state_lifecycle` are now done:

| File | Lines | Subsystem | Verdict |
|------|-------|-----------|---------|
| `src/bridge/sync_service.py` | 1,019 | bridge (active) | **Deferred** — largest active bridge file, formally deferred per §10 (holder lock singleton `BridgeServices:12-15` + `get_sync_service:424` + globals `ACTIVE_SYNC_RUNS:39`, 6 Protocols, `BridgeApi` 9-field wiring). Record size-only, revisit only after holder/state decoupling. |

Seam-audit note: the analyzer's "not flagged for monkeypatch" claim only scans `src/`, not `tests/` — `state_lifecycle` carried one `normalize_job_lifecycle_payload` monkeypatch seam + four private imports (`_index_lifecycle_entry_aliases`, `_initialize_carried_lifecycle_rows`, `_lifecycle_alias_index`, `_resolve_lifecycle_key`) and was split 2026-08-20 via the `_sl.` call-time pattern + retarget of the 4 privates to `state_lifecycle_identity`/`orchestration` (45/49 byte-identical + 4 seam diffs); `reporting_dedup` was the cleanest candidate (zero monkeypatch) and is now done (`12/12`).

Everything else ≥ 1,000 lines is **deferred (size-only)** per the rule — now **13 files** after both splits (down from 15). All are separate-scope subsystems (`src/source_discovery/` 2,087, 1,337, 1,263, 1,031, 1,017; `src/source_sync_*` 1,525, 1,345; `src/ship/` 1,341, 1,103) and non-subsystem modules (`fetch_incremental_sanity_benchmark` 1,171, `fetch_report_normalization` 1,167, `container_gateway` 1,084) plus the formally deferred `sync_service` 1,019 (bridge, see verdict). Sub-1,000 files (`_rendered_cards` 990, `registry_service` 963, `json_payloads` 913, `location` 910, `_windows` 900) are below the bar — do not split on size alone. No `src/jobs` file now exceeds 1,000.
...
| 8 | **Merge `fetcher_compat_runtime.py` into `jobs_fetcher.py`** (68 lines moved, no logic change) | Low | 2 | ~0 delta | **Done — WONTFIX keep separate** (per 2026-08-20 decision: facade budget `suite_contract_policy.py:558` 440 + 66 → 456 breach; `fetcher_compat_runtime.py:15` 5 wrappers `try/finally` isolate compat patching from thin `jobs_fetcher` facade) |
| 9 | **Unify root injection** — eliminate Points B (pipeline_stage_source_execution) and C (pipeline_source_loop fallback); all 4 root-dependent modules point to jobs_fetcher | Medium | 5 | ~100 | **Partial-Done** (2026-08-20 `18742ecb` consolidated 4 `root` vars into `pipeline_root.py:51` two-slot seam `bind_jobs_fetcher` + `bind_pipeline`; `pipeline_source_{progress,results}:7,17` now `require_pipeline_root`, but `jobs_fetcher:327` + `pipeline_stage_source_execution:40` + `loop:55` fallback remain as blessed **compatibility debt** per `pipeline_root.py:8-10`) |
...
|---|--------|------|-------------|--------|
| 11 | **Split `reporting_dedup_evidence.py`** (3,641 lines) into 5 sub-modules | Medium | Extract bundle_shapes, identity_quality, provider_static, review_queue, audit_gate | ✅ Done — **split 2026-08-20: 1,133 → 12-line coordinator** + 3 leaves (`dedup_evidence_review_queue.py:285`, `dedup_evidence_audit_gate_payload.py:302`, `dedup_evidence_bundle_report.py:596`), **12/12 byte-identical**; no longer a hotspot (dropped out of top-10; `dedup_evidence_bundle_report.py:596` is the only new large file in `src/jobs/common/`) |
| 11b | **Split `state_lifecycle.py`** (1,121 lines) into 5 sibling leaves | Medium | `state_lifecycle_{identity:120,availability:236,normalization:257,transitions:291,orchestration:321}` + coordinator 70 — identity, availability, normalization (`_sl.` seam for `normalize_job_lifecycle_payload`), transitions, orchestration | ✅ Done — **split 2026-08-20: 1,121 → 70-line coordinator** + 5 leaves, **45/49 byte-identical + 4 seam diffs** (`read_job_lifecycle_state`, `read_job_lifecycle_archive_state`, `write_job_lifecycle_archive_state` via `_sl.normalize_job_lifecycle_payload` + `AVAILABILITY_HISTORY_DAYS` const relocation with `fingerprint_url`); retargeted 4 private test imports (`_index_lifecycle_entry_aliases`, `_lifecycle_alias_index`, `_resolve_lifecycle_key` → `state_lifecycle_identity`, `_initialize_carried_lifecycle_rows` → `state_lifecycle_orchestration`); no longer a hotspot (dropped out of top-10; `0` jobs-core files >1k) |
| 12 | **Review `pipeline_run_setup.py`** (508 lines) for helper extraction | Low | Currently manageable, revisit if it grows | ⚠️ Revisit triggered (2026-08-19) — grew to **712 lines**, crossed the 500-line warning threshold (scanner score 55) — **deferred per §10** (500–1k not a bar; `pipeline_finalize 703` precedent) |
...
| Large files to split | **0 candidates** (when-to-split rule, §3F verdict — 13 files ≥1k) | **16** coordinator+leaves splits done (14 previous 2026-08-19 + **`reporting_dedup_evidence.py` 2026-08-20** `1,133→12` 12/12 + **`state_lifecycle.py` 2026-08-20** `1,121→38` 45/49 + 4 seam diffs); remaining >1,000: `sync_service.py` (1,019) — **formally deferred** (holder singleton + 6 Protocols + `RLock`, see §3F verdict) |
