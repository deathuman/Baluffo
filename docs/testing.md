...
**The local gate never runs mypy, and the reason is stage selection, not a missing hook.** `.pre-commit-config.yaml` *does* declare a `mypy` hook, but with `stages: [pre-push]`. `scripts/precommit_gate.py` invokes the framework as `pre_commit run --files …` / `--all-files` with no `--hook-stage`, which selects the **default `pre-commit` stage**; it then appends only `vulture` explicitly. So the three `pre-push`-only hooks (`mypy`, `eslint`, `vulture`) are never selected except for the hand-added `vulture` — `mypy` and `eslint` are dead configuration locally. A green `npm run lint:precommit` therefore says nothing about types, and `e696be64` landed two mypy errors on `main` that CI's `Lint` lane caught. Run `npm run typecheck:py` yourself before pushing Python changes, especially new test files, where `getattr(module, "x", None)` infers as `None` and reads as `"None" not callable`.

Do **not** fix this by simply adding `--hook-stage pre-push` to the gate. That also selects `eslint`, whose hook runs `npx eslint .` with `pass_filenames: false` — ignoring the file list the gate passes. `eslint.config.js` does not ignore `.venv/**` or `.container-frontend/**`, so the hook currently reports **7,118 errors**, of which 7,063 come from vendored/built JavaScript. Fix the `eslint.config.js` ignores first, then widen the stage; `eslint` is not wired into CI, `release:preflight`, or any hook either, so it is unenforced today.
