...
- GameDevMap recovery-cache scope is an explicit trial setting: `gamedevmap.activeAuditRecoveryCacheScope: "batch"` clears the in-memory recovery-body cache at each batch boundary while retaining wave-local reuse. The default remains `"run"`; compare candidate/output parity and repeated cgroup peaks before changing the default.
