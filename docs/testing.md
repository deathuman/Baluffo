...
**The local pre-commit gate does not run mypy.** `npm run lint:precommit` is a pre-commit-hook runner, and the hook set has no mypy hook; CI's `Lint` lane runs `npm run typecheck:py` separately. So a green local gate and a green `git push` (which runs `lint:precommit:ci`) can still land a mypy error on `main` — that is exactly how `e696be64` broke `Lint` with two `var-annotated`/`misc` errors in new test files. Run `npm run typecheck:py` yourself before pushing anything that adds or edits Python, especially new test files, where `getattr(module, "x", None)` infers as `None` and reads as `"None" not callable`.
