...
- Active fetch smoke should capture `/ops/storage-metrics` before and after the run window. Acceptance requires no active-run `/ops/storage-health` polling, no new runaway `jobs-fetch-tasks.json` payload growth during prep, and fewer-or-equal `admin-task-lifecycle.json` rewrites during prep compared with the prior 2-second hot-progress mirror behavior.
