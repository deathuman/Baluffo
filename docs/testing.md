...
- `tests/test_shared_utils_coercion.py` holds two ratchets over int coercion. `int(float("inf"))` raises `OverflowError`, which is **not** a `ValueError` subclass, so `except (TypeError, ValueError)` catches `nan` but lets `inf` escape. The name-gated ratchet scans `_INT_HELPER_NAMES` for `int()` without an `OverflowError` handler, and `_KNOWN_OVERFLOW_UNSAFE` is now empty. Because the defect is a property of the *body* and not the name, a second test compares body digests: a guarded helper's body reappearing unguarded under a name the ratchet ignores fails the run. Add a new coercion name to `_INT_HELPER_NAMES` rather than relying on the digest test alone.
