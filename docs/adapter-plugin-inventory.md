...
Provider coverage validation is recorded only from the staged provider source's own provider fetch evidence. A provider row with `keptCount > 0` may become `providerCoverageStatus="validated_provider"`, which means the provider source is real and usable. It does not mean the static source can be skipped or replaced. Repeated successful provider fetches may produce diagnostic `providerReplacementReadiness="ready_later"`, but static cleanup remains a separate, explicit redundant-static decision and this workflow does not mutate `REDUNDANT_STATIC_IF_PROVIDER`.
