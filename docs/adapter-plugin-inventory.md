...
- `src/jobs/adapters/static_detail.py` — removed facade; detail traversal planning lives in `static_listing.py`, detail heuristics in `static_detail_heuristics.py`, and runtime support in `static_runtime_support.py`.
...
- **`src/jobs_fetcher.py`** (lazy compat export table)
  Lazy compatibility export table for parser, adapter, registry, canonicalize, and dedup symbols surfaced through `src.jobs_fetcher` (merged from `src/jobs/fetcher_compat_exports.py`).
...
- `src/jobs/adapters/social_parsers.py` — removed facade. Reddit, X, Mastodon, and shared signal parsing live under `src/jobs/adapters/social_parser/` (`reddit_parser.py`, `x_parser.py`, `mastodon_parser.py`, `signals.py`), imported directly by `social.py`.
...
4. **Act on the outcome**:
   ...
   - **200 but 0 jobs**: Add or update a static plugin for that host. In the plugin: use `_heuristics.detect_no_openings(html)` to set `empty_confirmed` only when visible page text explicitly says the source has no open jobs; otherwise set `classification: needs_review` and `browserFallbackRecommended: true`. Weak labels such as `0 results` or `0 jobs` need visible jobs/careers context, and script/template/hidden text must not count as no-openings evidence. Use `_heuristics.detect_js_shell(html)`, `detect_outbound_ats_links(...)`, and listing/redirect signals to upgrade to `js_required`, `site_changed`, or `anti_bot_or_challenge` when the evidence is strong enough. If rows were fetched but all rows later canonical-drop, such as `missing_job_link`, keep the source in review instead of marking it as no-openings.
...
Source-policy recommendations are generated local review artifacts derived from completed-run `redundantStaticProposals`. The artifact keeps bounded per-pair history so safe, review, static-only, unstable-provider, and more-history evidence can be tracked across runs. `stable_safe_redundant` is still advisory only; it does not apply a source-policy change, mutate source registries, mutate sync snapshots, or change `REDUNDANT_STATIC_IF_PROVIDER`. Permanent source cleanup remains a later explicit milestone.

Manual migration identity backfill is an explicit Admin/API action for existing active or pending provider rows. It may add `migrationSourceIdentity` and Admin-owned link metadata to a supported provider row so provider coverage can associate that provider with an existing static/generic source. It is reversible only for links created by `admin_provider_link_backfill`; it does not modify the static source, create a permanent redundancy rule, hide/delete/reject/tombstone sources, mutate `REDUNDANT_STATIC_IF_PROVIDER`, add force-suppress behavior, or require Admin for normal runtime improvement.

### How to add new sources by family

- **Provider API (Greenhouse, Lever, Recruitee, Pinpoint, BambooHR, Workday, Breezy, JazzHR, Oracle HCM, etc.):** Add the source to the runtime registry (`data/source-registry-active.json` or via Admin -> Sources). The fetcher loads registry entries by adapter type; ensure the entry has the required fields (e.g. `slug` for Greenhouse, `api_url` for Lever/Recruitee/Pinpoint, `board_url` for Ashby/Breezy/JazzHR, `feed_url` for Personio, `pages` or `listing_url` for BambooHR/Workday migration sources, `listing_url` plus `base_url`/`site_path` for Oracle HCM CE sources). No change to `DEFAULT_SOURCE_LOADER_NAMES` is needed once the provider family itself exists. `personio_sources` is now registered through `src/jobs/adapters/plugins/provider_api/register.py` and exposed through the provider_api surface.
...
- **New community board / aggregator:** Add the parser and loader in `src/jobs/adapters/community/__init__.py` (e.g. `parse_gamejobs_html`, `parse_workwithindies_html`, `parse_8bitplay_html`, `parse_gracklehq_html`), and only touch `src/jobs_fetcher.py` if a legacy CLI compatibility re-export must stay available. Lazy compatibility export routing lives in `src/jobs_fetcher.py`, and root-backed wrapper seams live in `src/jobs/fetcher_compat_runtime.py`. Then add the loader name to `DEFAULT_SOURCE_LOADER_NAMES` and `SOURCE_REPORT_META` in `src/jobs_fetcher_registry.py`. Recent examples: `gamejobs`, `workwithindies`, `8bitplay`, `gracklehq`.
