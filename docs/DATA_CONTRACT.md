...
- **Directory audit metadata** may appear as top-level `directoryAuditSummaries` and `summary.directoryAudits` when enabled Gameprog, Gamesmap, sheet-directory, or opt-in web-search audits run or reuse a fresh artifact in the current process. Gameprog and sheet-directory audits run by default when their stages are enabled; Gamesmap still also requires `gamesmap.enabled=true`; the combined seed-careers/web-search audit appears only when `webSearch.activeAuditEnabled=true`. These additive diagnostics include cache hit, completion, audit duration, candidate/failure counts, timing totals, artifact size, top failure buckets, and adapter-owned boundary counts. They are not candidate registry fields and must not be copied into active, pending, or rejected source rows.
