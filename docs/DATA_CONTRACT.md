...
- **Directory audit metadata** may appear as top-level `directoryAuditSummaries` and `summary.directoryAudits` when enabled Gameprog, Gamesmap, or sheet-directory audits run or reuse a fresh artifact in the current process. These additive diagnostics include cache hit, completion, audit duration, candidate/failure counts, timing totals, artifact size, top failure buckets, and adapter-owned boundary counts. They are not candidate registry fields and must not be copied into active, pending, or rejected source rows.
