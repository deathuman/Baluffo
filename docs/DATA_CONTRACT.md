...
| `locations` | `Array<Object>` | Normalized structured location evidence for roles that expose multiple or richer location entries. |
...
- **Per-run evidence history:** `source-discovery-report.json` is a single-slot artifact, so every changed write snapshots terminal payloads (truthy `finishedAt`) into per-run history slots under `data/discovery-report-history/source-discovery-report-<runid-digest>-<UTC-stamp>.json.gz` — the run component is a hex digest of the runId (never the raw runId text), so a hostile payload cannot shape the filename; one slot per run, newest wins for repeated terminal writes of the same run, retention keeps the newest 48. A later run can never displace an earlier run's evidence (the same invariant as `data/fetch-report-history/` for `jobs-fetch-report.json`; shared implementation in `src/report_history_slots.py`). History files are best-effort and never block a report write.
