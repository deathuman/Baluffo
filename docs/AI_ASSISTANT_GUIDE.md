...
| **Desktop** | `src/ship/desktop_app/{launcher,startup}.py` | `src/ship/desktop_app/{launcher_flow,launcher_diagnostics,launcher_recovery,startup_ready,startup_watchdog,browser,session,_windows,config}.py`, `src/ship/`, `scripts/` |
...
| Bridge/API | `src/bridge/*.py` | `src/bridge/routes/{get_routes,post_routes,post_routes_admin,post_routes_local_data,post_routes_update}.py`; for ops/report/live-task work start with `src/bridge/ops_api.py`, `src/bridge/ops_history_projection.py`, `src/bridge/ops_task_live.py`, `src/bridge/ops_task_{fetch_live,discovery_live,projection}.py`, and `src/bridge/ops_live_payload.py` |
...
| Jobs pipeline / fetcher behavior | `src/jobs/pipeline.py`, `src/jobs/pipeline_{run_setup,execution_flow,finalize}.py`, `src/jobs/pipeline_{runtime_writers,runtime_summary}.py`, `src/jobs/pipeline_stage_source_execution.py`, `src/jobs/pipeline_source_{loop,results,progress}.py`, `src/jobs/pipeline_timing.py`, `src/jobs/state_{source_state,lifecycle,incremental}.py`, `src/jobs/state_source_{records,browser,migration}.py`, `src/jobs/reporting_{summary,queues,breakdowns,social}.py`, `src/jobs/fetcher_compat_{exports,runtime}.py`, other `src/jobs/*` leaf modules | `src/jobs_fetcher.py` only for CLI or compatibility-surface changes |
| Jobs page domain helpers | `frontend/jobs/domain/{query,feed,view}.js` | `frontend/jobs/domain.js` only when the stable export surface must change |
...
- `src/ship/desktop_app/launcher.py` and `src/ship/desktop_app/startup.py` are thin package-private orchestration/compat surfaces; runtime ownership belongs in `src/ship/desktop_app/launcher_{flow,diagnostics,recovery}.py` and `src/ship/desktop_app/startup_{ready,watchdog}.py`.
...
- `src/jobs/pipeline.py` is the stable package entrypoint; runtime setup lives in `src/jobs/pipeline_run_setup.py`, runtime/task-output ownership lives in `src/jobs/pipeline_runtime_{writers,summary}.py`, source execution compatibility stays rooted at `src/jobs/pipeline_stage_source_execution.py`, per-source loop/report/progress ownership lives in `src/jobs/pipeline_source_{loop,results,progress}.py`, and late-stage output/report assembly lives in `src/jobs/pipeline_finalize.py`.
...
- `src/jobs/state.py` is the stable jobs-state compatibility surface; source-state compatibility stays rooted at `src/jobs/state_source_state.py`, record/browser/migration ownership lives in `src/jobs/state_source_{records,browser,migration}.py`, lifecycle ownership lives in `src/jobs/state_lifecycle.py`, and cadence/freshness policy lives in `src/jobs/state_incremental.py`.
...
- `src/bridge/routes/post_routes.py` is the stable POST registration surface; route-family ownership lives in `src/bridge/routes/post_routes_{admin,local_data,update}.py`.
...
- `frontend/jobs/domain.js` is the stable Jobs domain export surface; implementation belongs in `frontend/jobs/domain/{query,feed,view}.js`.
