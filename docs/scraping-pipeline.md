...
- Provider/static overlap audit: completed fetch reports may include read-only `providerStaticOverlap` diagnostics for dynamically suppressed static rows. The audit uses existing source-state and current `sourceBundle` evidence when available; it does not force-run suppressed static sources, change suppression eligibility, or mutate registry rows. Static cleanup remains a separate explicit evidence-backed milestone.
