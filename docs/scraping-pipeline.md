...
- Dedup evidence: completed fetch reports may include read-only `dedupEvidence` diagnostics built from final canonical rows, `sourceBundle`, and dedup stats. Current-run `mergedCount` / `mergeReasonCounts` can be zero while `sourceBundleCollisionCount` remains nonzero because canonical rows may carry forward historical bundle evidence. This section explains source-bundle outliers, outlier reason counts, URL/source identity shape counts, risk reason counts, source-class composition, and risky merge examples such as same title/company across different meaningful locations or weak title/company-only evidence. Outlier reasons distinguish likely multi-location strong-identity bundles, location divergence without strong identity, provider/static disagreement, large unclassified source bundles, and sparse title/company-only bundles. Identity shapes distinguish shared job-detail URLs from shared listing/category URLs, many unique URLs with the same title, provider-ID-backed rows, and missing URL/provider-ID evidence. Shared URL evidence is not automatically safe when it looks like a listing/category page or carries category/open-application title caveats. `reviewQueueCounts` and `reviewQueue` provide capped, advisory samples for operators to inspect before lifecycle UX or dedup behavior changes; they do not persist review decisions and do not expose merge/unmerge controls. Dedup evidence does not change dedup decisions, lifecycle retention, source selection, or source-policy behavior.
