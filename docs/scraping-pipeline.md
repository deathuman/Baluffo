...
- Sheet-directory audit pilot: sheet-directory discovery can use the shared directory audit artifact when `sheetDirectory.activeAuditEnabled=true`. The default remains the legacy direct CSV scan. The audit records CSV fetch/parse/candidate timings, selected CSV URL, row counts, candidate counts, and failure buckets, then feeds the same provider/static/failure rows into the normal discovery queue.
