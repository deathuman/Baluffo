...
- Gameprog/Gamesmap directory audits: these adapters have opt-in HTTP-only directory audit artifacts (`activeAuditEnabled=false` by default). When enabled, they still return the same provider/static/failure rows and route candidates through the normal discovery queue. The final discovery report includes `directoryAuditSummaries` / `summary.directoryAudits` only for audits that ran or reused a fresh artifact in the current process, exposing cache hit, duration, artifact size, timing totals, top failure buckets, and adapter boundary counts for default-enablement evaluation.
