...
- Dedup evidence: completed fetch reports may include read-only `dedupEvidence` diagnostics built from final canonical rows, `sourceBundle`, and dedup stats. Current-run `mergedCount` / `mergeReasonCounts` can be zero while `sourceBundleCollisionCount` remains nonzero because canonical rows may carry forward historical bundle evidence. This section explains source-bundle outliers, outlier reason counts, risk reason counts, source-class composition, and risky merge examples such as same title/company across different meaningful locations or weak title/company-only evidence. Outlier reasons distinguish likely multi-location strong-identity bundles, location divergence without strong identity, provider/static disagreement, large unclassified source bundles, and sparse title/company-only bundles. It does not change dedup decisions, lifecycle retention, source selection, or source-policy behavior.
