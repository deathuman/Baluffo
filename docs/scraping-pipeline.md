...
- Seed-careers/web-search audit: enabled seed-careers and web-search stages write/reuse `data/web-search-discovery-audit.json` by default. Set `webSearch.activeAuditEnabled=false` to roll back to the direct seed-careers and web-search scanners. The audit records search/page-fetch timings, sub-stage counts, candidate counts, and failure buckets, then feeds the same provider/static/failure rows into the normal discovery queue.
