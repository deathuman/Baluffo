...
See also: `docs/DATA_CONTRACT.md` for report shapes and `docs/architecture-ai-map.md` for static adapter and Scrapy path. For static adapter edits, keep `src/jobs/adapters/static.py` as the compatibility surface and route implementation changes to the focused `static_{runtime,listing,detail,sources}.py` helpers. For pipeline/state edits, keep `src/jobs/pipeline.py` and `src/jobs/state.py` as stable roots and route implementation changes to `src/jobs/pipeline_{run_setup,execution_flow,finalize}.py` and `src/jobs/state_{source_state,lifecycle,incremental}.py`.
