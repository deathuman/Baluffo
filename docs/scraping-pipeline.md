...
The soak report also includes provider coverage link backfill diagnostics. These identify active provider rows that lack `migrationSourceIdentity` but appear to cover static/generic sources through exact redundant-static rules or provider-migration advisory evidence. The section is advisory only: it may show high-confidence backfill candidates, ambiguous static matches, and blocker examples, but it never writes `migrationSourceIdentity` or changes registry rows. Any actual migration identity backfill remains a later explicit milestone.
