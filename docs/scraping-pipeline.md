...
- Dedup evidence: completed fetch reports may include read-only `dedupEvidence` diagnostics built from final canonical rows, `sourceBundle`, and dedup stats. Current-run `mergedCount` / `mergeReasonCounts` can be zero while `sourceBundleCollisionCount` remains nonzero because canonical rows may carry forward historical bundle evidence. This section explains source-bundle outliers, risk reason counts, source-class composition, and risky merge examples such as same title/company across different meaningful locations or weak title/company-only evidence. It does not change dedup decisions, lifecycle retention, source selection, or source-policy behavior.
