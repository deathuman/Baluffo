...
- **Internal helpers:** `src/jobs/adapters/static_helpers.py` owns report bootstrap, runtime config, cached fetch/time-budget behavior, detail-link heuristics, detail-page processing, and the shared job-page gate used to reject regular pages as `dead_listing_page`. Keep new static internals there before growing `static.py` again.
...
- **Plugins:** `src/jobs/adapters/plugins/static/` — one module per site. All plugins below are registered in `register.py`:
  ...
  - Shared static plugins should reject obvious regular pages via the shared job-page gate and surface them as `dead_listing_page` rather than inventing a synthetic job from a slug or link title.
...
| Bridge API/runtime behavior | `src/admin_bridge.py` (⚠️ COMPOSITION ROOT - prefer `src/bridge/*.py` modules) | `frontend/admin/services.js`, `frontend/jobs/services.js`, `frontend/saved/services.js` |
...
| Bridge discovery operations | `src/bridge/discovery_service.py` | `src/admin_bridge.py`, `src/source_discovery/` (package), `src/source_discovery.py` (⚠️ CLI/legacy - use package) |
