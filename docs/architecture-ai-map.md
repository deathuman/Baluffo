...
**Admin page:** `frontend/admin/app.js` -> `runtime.js` -> `runtime/{composition,overview,events,state,view,effects,actions}.js`, `app/{auth,fetcher,discovery,sync}.js`, `app/registry/{ui,load,mutations}.js`, `app/ops/{format,task-state,health,bridge-status}.js`
  ...
  -> Action Center health signals read `/ops/fetch-kpis?view=summary`, **not** `/ops/health?view=ready`: the ready payload is built by `compute_ops_health_ready()` and carries neither `alerts` nor `kpis`, so `stale_fetch` and `failed_sources` could never fire from it. `render/sources.js` imports `ACTION_CENTER_ICONS` from `render/action-center.js` for its info glyph — a leaf-to-leaf import with no cycle
  -> The Action Center's first poll is enqueued on the composition root's serial startup bridge lane (`enqueueAdminStartupBridgeTask`, injected as `enqueueStartupTask`). That route is startup-heavy and the bridge serves one gateway, so a racing first poll would overlap the bootstrap heavy loads; the injection defaults to an immediate call so unit fixtures keep the direct behaviour
  -> Ops tab badges render three distinct states: `loaded: true` (count), `loaded: false` without an `error` code (pending `...`), and `loaded: false` **with** an error code (terminal `-`). Only the error code separates "still fetching" from "will never arrive"
