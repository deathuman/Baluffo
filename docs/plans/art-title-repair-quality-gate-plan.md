...
- Final shipped-artifact quality gate tooling and a fresh live-source pipeline validation were completed on 2026-05-30. Static exact-category recovery and the follow-up static container/filter artifact hardening also landed on 2026-05-30; the gate now tracks `exactCategoryTitleLeaks` and `staticContainerTitleLeaks`, and the targeted static live artifact `_out/static-container-filter-targeted4-20260530-live` passed with both counts at zero.
