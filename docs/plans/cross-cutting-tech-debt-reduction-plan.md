...
**43 source instances** of `except Exception`/`suppress(Exception)` remain across **17 source files**:

- 52 total `src` suppression comments, including 44 BLE001 suppressions guarded by budget
- 43 source broad-catch sites remain after excluding one non-broad BLE001 suppression
