...
- **Updater pure constant root-binding reduction:** `desktop_update_shared.py` and `desktop_update_state.py` now read `DESKTOP_UPDATE_CHANNEL`, `DESKTOP_UPDATE_SCHEMA_VERSION`, `DESKTOP_UPDATER_VERSION`, and `PUBLIC_KEYS_FILE` from `desktop_update_manifest.py`, reducing tracked updater root dependencies from 111 to 107 and references from 249 to 241.
