...
**73 source instances** of `except Exception`/`suppress(Exception)` remain across **41 source files**:

- 82 total `src` suppression comments, including 74 BLE001 suppressions guarded by budget
- 73 source broad-catch sites remain after excluding one non-broad BLE001 suppression
