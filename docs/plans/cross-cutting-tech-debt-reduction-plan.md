...
**38 source instances** of `except Exception`/`suppress(Exception)` remain across **16 source files**:

- 47 total `src` suppression comments, including 39 BLE001 suppressions guarded by budget
- 38 source broad-catch sites remain after excluding one non-broad BLE001 suppression
