...
**5 source instances** of `except Exception`/`suppress(Exception)` remain across **4 source files**:

- 14 total `src` suppression comments, including 6 BLE001 suppressions guarded by budget
- 5 source broad-catch sites remain after excluding one non-broad BLE001 suppression
...
| `src/jobs/pipeline_source_loop.py` | 1 | Per-source adapter failure boundary |
| `src/jobs/pipeline_source_results.py` | 1 | Per-source report failure boundary |
