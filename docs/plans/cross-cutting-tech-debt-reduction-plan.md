...
- `npm run test:py:extended` passed: `3574 passed, 1 skipped`
...
| 11 | Replace `except Exception` in low-risk files (post_routes_update, adapters, shared) | §4 | ~15 files | Partial | Update POST route, jobs transport, metric-only route, retained-log, helper fallback, optional-certifi, console-encoding, and profile-summary slices completed |
