...
**11 source instances** of `except Exception`/`suppress(Exception)` remain across **9 source files**:

- 20 total `src` suppression comments, including 12 BLE001 suppressions guarded by budget
- 11 source broad-catch sites remain after excluding one non-broad BLE001 suppression
