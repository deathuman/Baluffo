...
- `desktop_updater.py` helper root dependencies are guarded by `desktop_updater_root_dependency_inventory.py`; current inventory is 24 names / 38 references across UI, release, and install leaves after direct local constant/stdlib binding, same-module install status/rollback helper, install state/shared helper cleanup, and release helper lookup cleanup, with direct and nested test-proven facade monkeypatch seams classified separately.
