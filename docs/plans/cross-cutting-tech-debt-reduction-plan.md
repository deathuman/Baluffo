...
**79 source instances** of `except Exception`/`suppress(Exception)` remain across **43 source files**:

- 88 total `src` suppression comments, including 80 BLE001 suppressions guarded by budget
- 79 source broad-catch sites remain after excluding one non-broad BLE001 suppression
