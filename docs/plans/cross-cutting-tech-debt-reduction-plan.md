...
**21 source instances** of `except Exception`/`suppress(Exception)` remain across **14 source files**:

- 30 total `src` suppression comments, including 22 BLE001 suppressions guarded by budget
- 21 source broad-catch sites remain after excluding one non-broad BLE001 suppression
