...
**3 source instances** of `except Exception`/`suppress(Exception)` remain across **2 source files**:

- 12 total `src` suppression comments, including 4 BLE001 suppressions guarded by budget
- 3 source broad-catch sites remain after excluding one non-broad BLE001 suppression
