...
- `update_manager` has runtime consumers inside `src/ship/` (`runtime_launcher.py`, `desktop_updater_install.py`) plus script/packaging consumers. The issue is parallel updater architecture and root/re-export facades, not that `update_manager` is build-time-only.
...
- `desktop_update_shared.py` now imports current-version repair helpers from update-manager leaves instead of the `update_manager` facade; launcher and installer runtime coupling remain.
