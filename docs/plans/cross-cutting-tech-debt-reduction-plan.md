...
**80 source instances** of `except Exception`/`suppress(Exception)` remain across **44 source files**:

- 89 total `src` suppression comments, including 81 BLE001 suppressions guarded by budget
- 80 source broad-catch sites remain after excluding one non-broad BLE001 suppression
