...
| Fetch report normalization duplicated | P0 | Partial | Shared-compatible task-progress, socialSummary, timingSummary, bridge source-row enrichment, and jobs/bridge source-row base helpers extracted while preserving bridge/jobs shape differences. Deeper source-row enrichment convergence remains open. |
