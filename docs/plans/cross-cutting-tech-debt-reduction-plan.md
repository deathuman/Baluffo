...
**49 source instances** of `except Exception`/`suppress(Exception)` remain across **23 source files**:

- 58 total `src` suppression comments, including 50 BLE001 suppressions guarded by budget
- 49 source broad-catch sites remain after excluding one non-broad BLE001 suppression
