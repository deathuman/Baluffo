...
| Fetch report normalization duplicated | P0 | Partial | Shared-compatible task-progress, socialSummary, timingSummary, bridge source-row enrichment, jobs/bridge source-row base helpers, jobs source-row field enrichment, and jobs stage/loss/detail-stat helpers extracted while preserving bridge/jobs shape differences. Deeper jobs-only taxonomy/detail item orchestration remains open. |
