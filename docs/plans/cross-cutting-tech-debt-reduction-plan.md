...
- **Updater pure constant root-binding reduction:** `desktop_update_shared.py` and `desktop_update_state.py` now read `DESKTOP_UPDATE_CHANNEL`, `DESKTOP_UPDATE_SCHEMA_VERSION`, `DESKTOP_UPDATER_VERSION`, and `PUBLIC_KEYS_FILE` from `desktop_update_manifest.py`; remaining pure desktop update constants moved to `desktop_update_constants.py` and are read directly by shared/state/service leaves. Tracked updater root dependencies dropped from 111 to 80 and references from 249 to 209 while keeping `desktop_update.py` import-compatible.
