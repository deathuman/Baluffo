...
**59 source instances** of `except Exception`/`suppress(Exception)` remain across **31 source files**:

- 68 total `src` suppression comments, including 60 BLE001 suppressions guarded by budget
- 59 source broad-catch sites remain after excluding one non-broad BLE001 suppression
