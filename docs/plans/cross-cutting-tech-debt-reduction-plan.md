...
- `desktop_updater.py` helper root dependencies are guarded by `desktop_updater_root_dependency_inventory.py`; current inventory is 40 names / 81 references across UI, release, and install leaves after direct local constant/stdlib binding cleanup, with test-proven facade monkeypatch seams classified separately.
