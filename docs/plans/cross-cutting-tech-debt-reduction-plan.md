...
**24 source instances** of `except Exception`/`suppress(Exception)` remain across **15 source files**:

- 33 total `src` suppression comments, including 25 BLE001 suppressions guarded by budget
- 24 source broad-catch sites remain after excluding one non-broad BLE001 suppression
