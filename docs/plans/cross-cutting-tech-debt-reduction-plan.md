...
**13 source instances** of `except Exception`/`suppress(Exception)` remain across **10 source files**:

- 22 total `src` suppression comments, including 14 BLE001 suppressions guarded by budget
- 13 source broad-catch sites remain after excluding one non-broad BLE001 suppression
