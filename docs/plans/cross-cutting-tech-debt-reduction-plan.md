...
- `desktop_updater.py` helper root dependencies are guarded by `desktop_updater_root_dependency_inventory.py`; current inventory is 21 names / 35 references across UI, release, and install leaves after direct local constant/stdlib binding, same-module install status/rollback helper, install state/shared helper cleanup, release helper lookup cleanup, and UI helper self-call cleanup, with direct and nested test-proven facade monkeypatch seams classified separately.
