...
- `desktop_updater.py` helper root dependencies are guarded by `desktop_updater_root_dependency_inventory.py`; current inventory is 1 name / 8 references across UI, release, and install leaves after direct local constant/stdlib binding, same-module install status/rollback helper, install state/shared helper cleanup, release helper lookup cleanup, release manifest/hash/download shared-helper cleanup, install startup health-fetch cleanup, private install helper cleanup, UI helper self-call cleanup, and install non-monkeypatch helper cleanup. The remaining helper root dependency is the explicit install-time `update_manager` compatibility namespace.
...
- The desktop updater helper facade still uses the root-injection pattern (`module.root = sys.modules[__name__]`); the desktop update shared/state facade injection has been removed after the root-dependency inventory reached zero.
