...
- **Updater helper root-dependency inventory:** `tools/repo_health/desktop_updater_root_dependency_inventory.py` classifies all `module.<name>` / `getattr(module, "...")` helper root dependencies across desktop updater UI/release/install leaves and guardrails fail on dependency/reference count drift or unclassified root-binding usage. It currently records 30 dependencies and 44 references after direct local constant/stdlib binding, same-module install status/rollback helper, and install state/shared helper cleanup, and now annotates direct and nested test-proven facade monkeypatch seams so helper root reduction can continue in classified batches rather than by grep-only deletion.
...
- `desktop_updater.py` helper root dependencies are guarded by `desktop_updater_root_dependency_inventory.py`; current inventory is 30 names / 44 references across UI, release, and install leaves after direct local constant/stdlib binding, same-module install status/rollback helper, and install state/shared helper cleanup, with direct and nested test-proven facade monkeypatch seams classified separately.
