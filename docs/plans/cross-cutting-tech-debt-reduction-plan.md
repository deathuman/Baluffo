...
**9 source instances** of `except Exception`/`suppress(Exception)` remain across **8 source files**:

- 18 total `src` suppression comments, including 10 BLE001 suppressions guarded by budget
- 9 source broad-catch sites remain after excluding one non-broad BLE001 suppression
...
| `src/ship/runtime_launcher.py` | 1 | Runtime launcher CLI failure envelope |
