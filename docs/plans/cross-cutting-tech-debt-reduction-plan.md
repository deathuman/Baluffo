...
**27 source instances** of `except Exception`/`suppress(Exception)` remain across **16 source files**:

- 36 total `src` suppression comments, including 28 BLE001 suppressions guarded by budget
- 27 source broad-catch sites remain after excluding one non-broad BLE001 suppression
