...
**71 source instances** of `except Exception`/`suppress(Exception)` remain across **40 source files**:

- 80 total `src` suppression comments, including 72 BLE001 suppressions guarded by budget
- 71 source broad-catch sites remain after excluding one non-broad BLE001 suppression
