...
| admin_bridge legacy globals | P0 | Partial | 5-way root injection seam now has explicit coverage. `BridgeServices` holder owns sync config, sync, desktop-update, registry, discovery, and pipeline service state while mirroring legacy globals. Broader root-injection cleanup remains open. |
...
- **BridgeServices sync-config holder migration:** `BridgeServices` now owns the current sync config mirror while `admin_bridge.SYNC_CONFIG` remains import-compatible and runtime path reconfiguration clears both holder and legacy state.
...
| 12 | Service holder dataclass for admin_bridge singletons | §2 | 6 files | Done | Desktop-update, registry, sync config, sync, discovery, and pipeline holders completed; legacy globals still mirrored for compatibility |
