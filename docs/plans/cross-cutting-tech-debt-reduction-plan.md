...
- **3-way root injection seam**: sets `sys.modules[__name__]` onto `admin_entrypoint_services_mod.root`, `admin_registry_api_mod.root`, `admin_task_runtime_mod.root`; `admin_entrypoint_api.py` and `admin_entrypoint_runtime.py` now receive root explicitly
...
| 18 | Replace root injection seam with explicit dependency passing | §2 | 6 files | High | Started: `admin_entrypoint_api.py` and `admin_entrypoint_runtime.py` now receive root explicitly; 3 injected modules remain |
