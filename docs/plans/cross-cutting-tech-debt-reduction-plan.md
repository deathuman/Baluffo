...
- `npm run test:py:extended` passed: `3595 passed, 1 skipped`
...
| 11 | Replace `except Exception` in low-risk files (post_routes_update, adapters, shared) | §4 | ~15 files | Partial | Update POST route, jobs transport, metric-only route, retained-log, helper fallback, source-discovery browser fallback setup, container gateway termination cleanup, ship update-manager CLI cleanup, optional-certifi, console-encoding, profile-summary, registry journal compaction, source-sync remote timing/idempotency, source-sync shard pull cleanup, SQLite write rollback cleanup, dev-supervisor POSIX kill cleanup, update-manager apply rollback cleanup, and desktop updater install rollback cleanup slices completed |
