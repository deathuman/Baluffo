...
**45 source instances** of `except Exception`/`suppress(Exception)` remain across **19 source files**:

- 54 total `src` suppression comments, including 46 BLE001 suppressions guarded by budget
- 45 source broad-catch sites remain after excluding one non-broad BLE001 suppression
