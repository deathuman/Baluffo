...
**46 source instances** of `except Exception`/`suppress(Exception)` remain across **20 source files**:

- 55 total `src` suppression comments, including 47 BLE001 suppressions guarded by budget
- 46 source broad-catch sites remain after excluding one non-broad BLE001 suppression
