...
| Fetch report normalization duplicated | P0 | Partial | Shared-compatible task-progress, socialSummary, timingSummary, bridge source-row enrichment, jobs/bridge source-row base helpers, jobs source-row field enrichment, jobs stage/loss/detail-stat helpers, and jobs detail-list/provider/site-changed/dynamic redundant-provider helpers extracted while preserving bridge/jobs shape differences. Remaining jobs-only work is zero-kept taxonomy classification. |
...
- **Fetch-report jobs stage/loss/detail-stat extraction:** jobs stage timing, loss accounting, and detail stats normalization now live in `src/shared/fetch_report_normalization.py`; jobs source-report normalization keeps only taxonomy classification and item orchestration local.
- **Fetch-report jobs detail orchestration extraction:** jobs detail-list normalization, detail item provider-migration fields, dead-listing fields, and site-changed URL surface now live in `src/shared/fetch_report_normalization.py`; jobs source-report normalization keeps zero-kept taxonomy classification local.
- **Fetch-report dynamic redundant-provider field extraction:** dynamic redundant-provider source-policy field enrichment now lives in `src/shared/fetch_report_normalization.py`; jobs source-report normalization delegates to the shared helper while keeping zero-kept taxonomy classification local.
