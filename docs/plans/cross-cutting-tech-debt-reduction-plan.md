...
**84 source instances** of `except Exception`/`suppress(Exception)` remain across **46 source files**:

- 93 total `src` suppression comments, including 85 BLE001 suppressions guarded by budget
- 84 source broad-catch sites remain after excluding one non-broad BLE001 suppression
...
| `src/bridge/source_checker.py` | 3 | Source-check embedded fetch isolation |
