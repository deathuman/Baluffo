...
| Fetch report normalization duplicated | P0 | Partial | Shared-compatible task-progress, socialSummary, timingSummary, bridge source-row enrichment, jobs/bridge source-row base helpers, and jobs source-row field enrichment helpers extracted while preserving bridge/jobs shape differences. Deeper jobs-only taxonomy/detail convergence remains open. |
