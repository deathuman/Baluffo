...
- **BridgeServices sync-config holder migration:** `BridgeServices` now owns the current sync config mirror and refresh assignment while `admin_bridge.SYNC_CONFIG` remains import-compatible and runtime path reconfiguration clears both holder and legacy state; explicit `global SYNC_CONFIG` declarations are guardrailed out of the refresh path.
