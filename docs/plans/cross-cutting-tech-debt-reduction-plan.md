...
- **BridgeServices discovery holder migration:** `BridgeServices` now owns the discovery service instance/path tuple/lock while `admin_bridge` keeps legacy `_DISCOVERY_SERVICE*` globals mirrored for compatibility.
- **BridgeServices pipeline holder migration:** `BridgeServices` now owns the pipeline service instance/lock while `admin_bridge` keeps the legacy `_PIPELINE_SERVICE` global mirrored for compatibility.
...
- 10 total `src` suppression comments, including 2 BLE001 suppressions guarded by budget
- 2 source broad-catch sites remain
