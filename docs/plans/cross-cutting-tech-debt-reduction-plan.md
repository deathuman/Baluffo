...
**65 source instances** of `except Exception`/`suppress(Exception)` remain across **37 source files**:

- 74 total `src` suppression comments, including 66 BLE001 suppressions guarded by budget
- 65 source broad-catch sites remain after excluding one non-broad BLE001 suppression
