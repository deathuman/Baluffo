...
**51 source instances** of `except Exception`/`suppress(Exception)` remain across **25 source files**:

- 60 total `src` suppression comments, including 52 BLE001 suppressions guarded by budget
- 51 source broad-catch sites remain after excluding one non-broad BLE001 suppression
