...
**52 source instances** of `except Exception`/`suppress(Exception)` remain across **26 source files**:

- 61 total `src` suppression comments, including 53 BLE001 suppressions guarded by budget
- 52 source broad-catch sites remain after excluding one non-broad BLE001 suppression
