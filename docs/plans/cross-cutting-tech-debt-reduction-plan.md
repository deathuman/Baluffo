...
**44 source instances** of `except Exception`/`suppress(Exception)` remain across **18 source files**:

- 53 total `src` suppression comments, including 45 BLE001 suppressions guarded by budget
- 44 source broad-catch sites remain after excluding one non-broad BLE001 suppression
