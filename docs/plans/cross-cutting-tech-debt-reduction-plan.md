...
**67 source instances** of `except Exception`/`suppress(Exception)` remain across **38 source files**:

- 76 total `src` suppression comments, including 68 BLE001 suppressions guarded by budget
- 67 source broad-catch sites remain after excluding one non-broad BLE001 suppression
