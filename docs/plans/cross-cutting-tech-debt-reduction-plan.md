...
- **2-way root injection seam**: sets `sys.modules[__name__]` onto `admin_entrypoint_services_mod.root`, `admin_task_runtime_mod.root`; `admin_entrypoint_api.py`, `admin_entrypoint_runtime.py`, and `admin_registry_api.py` now receive root explicitly
...
| 18 | Replace root injection seam with explicit dependency passing | §2 | 6 files | High | Started: `admin_entrypoint_api.py`, `admin_entrypoint_runtime.py`, and `admin_registry_api.py` now receive root explicitly; 2 injected modules remain |
