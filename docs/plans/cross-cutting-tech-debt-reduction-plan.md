...
**81 source instances** of `except Exception`/`suppress(Exception)` remain across **45 source files**:

- 90 total `src` suppression comments, including 82 BLE001 suppressions guarded by budget
- 81 source broad-catch sites remain after excluding one non-broad BLE001 suppression
