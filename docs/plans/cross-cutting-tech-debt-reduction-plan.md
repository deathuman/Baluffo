...
**40 source instances** of `except Exception`/`suppress(Exception)` remain across **17 source files**:

- 49 total `src` suppression comments, including 41 BLE001 suppressions guarded by budget
- 40 source broad-catch sites remain after excluding one non-broad BLE001 suppression
