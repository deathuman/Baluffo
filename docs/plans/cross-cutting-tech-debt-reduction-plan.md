...
- **Fetch-report jobs source-row field enrichment extraction:** jobs browser escalation, cache/http/listing metadata, structured-migration, browser-fallback, and board/subsource cache-count enrichment now live in `src/shared/fetch_report_normalization.py`; jobs normalization keeps taxonomy, detail, dynamic redundant-provider, and site-changed URL logic local.
...
- **Fetch-report jobs detail orchestration extraction:** jobs detail-list normalization, detail item provider-migration fields, dead-listing fields, and site-changed URL surface now live in `src/shared/fetch_report_normalization.py`; jobs source-report normalization keeps zero-kept taxonomy classification and dynamic redundant-provider source-policy fields local.
