...
**47 source instances** of `except Exception`/`suppress(Exception)` remain across **21 source files**:

- 56 total `src` suppression comments, including 48 BLE001 suppressions guarded by budget
- 47 source broad-catch sites remain after excluding one non-broad BLE001 suppression
