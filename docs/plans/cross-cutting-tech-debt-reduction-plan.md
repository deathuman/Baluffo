...
**6 source instances** of `except Exception`/`suppress(Exception)` remain across **5 source files**:

- 15 total `src` suppression comments, including 7 BLE001 suppressions guarded by budget
- 6 source broad-catch sites remain after excluding one non-broad BLE001 suppression
...
| `src/scrapers/runner.py` | 1 | Scrapy crawl failure envelope |
