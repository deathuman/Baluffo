...
|------|----------|----------------|----------------------------|
| BridgeApi god object | P0 | Partial | Current-task default payload builders merged; source-derived field classification guardrail added and hardened for dynamic API lookups; pipeline task, app, sync GET, admin bootstrap GET, ops status GET, and update POST routes now use narrow capability protocols instead of the full `BridgeApi` type. Field deletion/split still open. |
...
4. **Route-scoped context/capabilities:** Add narrow route capability contracts or a `RouteContext` frozen dataclass that exposes only the fields routes actually read. Pipeline task, app, sync GET, admin bootstrap GET, ops status GET, and update POST routes now use narrow capability protocols.
