...
**74 source instances** of `except Exception`/`suppress(Exception)` remain across **41 source files**:

- 83 total `src` suppression comments, including 75 BLE001 suppressions guarded by budget
- 74 source broad-catch sites remain after excluding one non-broad BLE001 suppression
...
| `src/bridge/server/handler.py` | 3 | Server boundary/timing fallbacks |
