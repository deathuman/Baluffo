...
- `update_manager` has a runtime consumer inside `src/ship/` (`runtime_launcher.py`) plus script/packaging consumers. The issue is parallel updater architecture and root/re-export facades, not that `update_manager` is build-time-only.
...
- `desktop_update_shared.py` now imports current-version repair helpers from update-manager leaves instead of the `update_manager` facade; `desktop_updater_install.py` imports backup/migration helpers from update-manager leaves while preserving its patch namespace; launcher runtime coupling remains.
