...
**121 instances** of `except Exception`/`suppress(Exception)` across **62 source/tool files**:

- 99 with explicit annotation or `suppress(Exception)`, including 95 broad-catch `src` BLE001 sites and a guarded `src` BLE001 budget of 96
- 22 without any annotation
