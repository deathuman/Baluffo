...
**15 source instances** of `except Exception`/`suppress(Exception)` remain across **11 source files**:

- 24 total `src` suppression comments, including 16 BLE001 suppressions guarded by budget
- 15 source broad-catch sites remain after excluding one non-broad BLE001 suppression
