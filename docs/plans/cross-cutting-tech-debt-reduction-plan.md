...
- **BridgeApi route leaf import guardrail:** repo guardrails now fail when extracted route leaves under `src/bridge/routes/` import or type against the full `BridgeApi`; only the public `get_routes.py` and `post_routes.py` delegators remain allowed full-API compatibility entrypoints.
