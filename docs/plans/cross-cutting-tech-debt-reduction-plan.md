...
**8 source instances** of `except Exception`/`suppress(Exception)` remain across **7 source files**:

- 17 total `src` suppression comments, including 9 BLE001 suppressions guarded by budget
- 8 source broad-catch sites remain after excluding one non-broad BLE001 suppression
...
| `src/bridge/sync_task_flow.py` | 1 | Sync task failure summary boundary |
...
| `src/ship/packaged_smoke/orchestrator.py` | 1 | Packaged smoke failure envelope |
