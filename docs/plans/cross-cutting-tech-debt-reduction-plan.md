...
- `desktop_updater.py` helper root dependencies are guarded by `desktop_updater_root_dependency_inventory.py`; current inventory is 11 names / 22 references across UI, release, and install leaves after direct local constant/stdlib binding, same-module install status/rollback helper, install state/shared helper cleanup, release helper lookup cleanup, release manifest/hash/download shared-helper cleanup, install startup health-fetch cleanup, UI helper self-call cleanup, and install non-monkeypatch helper cleanup. All remaining helper root dependencies are classified as test-proven facade/helper monkeypatch or `mock.patch.object` seams.
