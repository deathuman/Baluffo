...
- **Updater pure helper root-binding reduction:** `desktop_update_shared.py` and `desktop_update_state.py` now read `DESKTOP_UPDATE_CHANNEL`, `DESKTOP_UPDATE_SCHEMA_VERSION`, `DESKTOP_UPDATER_VERSION`, and `PUBLIC_KEYS_FILE` from `desktop_update_manifest.py`; remaining pure desktop update constants moved to `desktop_update_constants.py`; manifest crypto key classes and version comparison now come from their owning leaves. Tracked updater root dependencies dropped from 111 to 77 and references from 249 to 206 while keeping `desktop_update.py` import-compatible.
