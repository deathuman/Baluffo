...
**29 source instances** of `except Exception`/`suppress(Exception)` remain across **16 source files**:

- 38 total `src` suppression comments, including 30 BLE001 suppressions guarded by budget
- 29 source broad-catch sites remain after excluding one non-broad BLE001 suppression
