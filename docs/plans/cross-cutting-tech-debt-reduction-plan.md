...
**48 source instances** of `except Exception`/`suppress(Exception)` remain across **22 source files**:

- 57 total `src` suppression comments, including 49 BLE001 suppressions guarded by budget
- 48 source broad-catch sites remain after excluding one non-broad BLE001 suppression
