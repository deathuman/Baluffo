...
**7 source instances** of `except Exception`/`suppress(Exception)` remain across **6 source files**:

- 16 total `src` suppression comments, including 8 BLE001 suppressions guarded by budget
- 7 source broad-catch sites remain after excluding one non-broad BLE001 suppression
