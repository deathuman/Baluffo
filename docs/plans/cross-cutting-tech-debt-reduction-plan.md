...
**17 source instances** of `except Exception`/`suppress(Exception)` remain across **12 source files**:

- 26 total `src` suppression comments, including 18 BLE001 suppressions guarded by budget
- 17 source broad-catch sites remain after excluding one non-broad BLE001 suppression
