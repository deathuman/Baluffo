...
| admin_bridge legacy globals | P0 | Partial | 5-way root injection seam now has explicit coverage. `BridgeServices` holder owns sync config, sync, desktop-update, registry, discovery, and pipeline service state; desktop-update, registry, discovery, and pipeline service root mirrors have been removed, while sync still mirrors legacy globals for compatibility. Explicit `global SYNC_CONFIG` refresh declarations are removed. Broader root-injection cleanup remains open. |
...
- **Remaining mutable singleton mirror** with per-service `RLock`: `_SYNC_SERVICE` + companion data-dir variable; desktop-update, registry, discovery, and pipeline service mirrors have been removed
...
- Move remaining `_SYNC_SERVICE` singleton mirror into explicit holder objects passed through `BridgeApi` or `RuntimeConfig`; `_DESKTOP_UPDATE_SERVICE*`, `_REGISTRY_SERVICE*`, `_DISCOVERY_SERVICE*`, and `_PIPELINE_SERVICE` are already removed
