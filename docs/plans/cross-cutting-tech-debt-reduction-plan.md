...
**78 source instances** of `except Exception`/`suppress(Exception)` remain across **42 source files**:

- 87 total `src` suppression comments, including 79 BLE001 suppressions guarded by budget
- 78 source broad-catch sites remain after excluding one non-broad BLE001 suppression
...
| `src/ship/packaged_smoke/runtime_wait.py` | 4 | Packaged smoke best-effort probes |
