...
| admin_bridge legacy globals | P0 | Partial | 5-way root injection seam now has explicit coverage. `BridgeServices` holder exists and owns sync, desktop-update, and registry service state while mirroring legacy globals. Discovery and pipeline singleton migration remain open. |
