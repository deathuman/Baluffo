...
| admin_bridge legacy globals | P0 | Partial | 5-way root injection seam now has explicit coverage. `BridgeServices` holder owns sync config, sync, desktop-update, registry, discovery, and pipeline service state; registry and pipeline service root mirrors have been removed, while sync, desktop-update, and discovery still mirror legacy globals for compatibility. Explicit `global SYNC_CONFIG` refresh declarations are removed. Broader root-injection cleanup remains open. |
...
- **Remaining mutable singleton mirrors** with per-service `RLock`: `_SYNC_SERVICE`, `_DISCOVERY_SERVICE`, `_DESKTOP_UPDATE_SERVICE` + companion data-dir/path variables; registry and pipeline service mirrors have been removed
...
- Move remaining `_SYNC_SERVICE`, `_DISCOVERY_SERVICE`, and `_DESKTOP_UPDATE_SERVICE` singleton mirrors into explicit holder objects passed through `BridgeApi` or `RuntimeConfig`; `_REGISTRY_SERVICE*` and `_PIPELINE_SERVICE` are already removed
