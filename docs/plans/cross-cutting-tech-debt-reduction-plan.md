...
**10 source instances** of `except Exception`/`suppress(Exception)` remain across **9 source files**:

- 19 total `src` suppression comments, including 11 BLE001 suppressions guarded by budget
- 10 source broad-catch sites remain after excluding one non-broad BLE001 suppression
...
| `src/ship/desktop_app/launcher.py` | 1 | Desktop launch user-facing failure |
