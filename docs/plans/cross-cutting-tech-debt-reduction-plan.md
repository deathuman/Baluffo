...
**61 source instances** of `except Exception`/`suppress(Exception)` remain across **33 source files**:

- 70 total `src` suppression comments, including 62 BLE001 suppressions guarded by budget
- 61 source broad-catch sites remain after excluding one non-broad BLE001 suppression
