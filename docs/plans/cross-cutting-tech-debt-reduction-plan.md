...
- **Updater update-manager facade consumer inventory:** `tools/repo_health/update_manager_facade_inventory.py` classifies all current Python imports of `src.ship.update_manager` and guardrails fail on unclassified consumers, count drift, or new runtime imports outside the compatibility allowlist. The apply interrupt exception-ratchet test now imports update-manager leaves directly; the main compatibility/API test remains the only facade consumer.
