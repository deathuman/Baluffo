...
- `desktop_updater.py` helper root dependencies are guarded by `desktop_updater_root_dependency_inventory.py`; current inventory is 38 names / 65 references across UI, release, and install leaves after direct local constant/stdlib binding cleanup plus same-module install status/rollback helper cleanup, with test-proven facade monkeypatch seams classified separately.
