...
4. **Route-scoped context/capabilities:** Add narrow route capability contracts or a `RouteContext` frozen dataclass that exposes only the fields routes actually read. Pipeline task, app, sync GET, admin bootstrap GET, admin ops-tab counts GET, desktop local-data GET/POST, registry GET, ops status GET, ops diagnostics GET, fetch-report GET, registry-conflicts GET, source-policy GET, discovery GET, and update POST routes now use narrow capability protocols.
