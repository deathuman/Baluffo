...
| admin_bridge legacy globals | P0 | Partial | 5-way root injection seam now has explicit coverage. `BridgeServices` holder exists and owns sync, desktop-update, registry, and discovery service state while mirroring legacy globals. Pipeline singleton migration remains open. |
...
| 12 | Service holder dataclass for admin_bridge singletons | §2 | 6 files | Partial | Desktop-update, registry, sync, and discovery holders completed; pipeline remains |
