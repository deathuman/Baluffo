...
**34 source instances** of `except Exception`/`suppress(Exception)` remain across **16 source files**:

- 43 total `src` suppression comments, including 35 BLE001 suppressions guarded by budget
- 34 source broad-catch sites remain after excluding one non-broad BLE001 suppression
