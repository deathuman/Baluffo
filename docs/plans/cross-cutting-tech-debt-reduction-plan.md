...
> - **Last updated:** 2026-06-17 — validated against current source; multiple P0 implementation slices completed; macOS platform work deferred by product priority; stale footprint counts and unsafe acceptance criteria corrected; _as_dict/_as_list remains P3 (4 callers, contained refactor)
...
| get_routes.py monolith | P0 | Partial | Partial JSON parser, provider-coverage link backfill, and registry source table compaction extracted with tests. Dispatch split and caches still open. |
...
| Bare `except Exception` | P0 | Partial | Two URL parsing fallbacks narrowed and update POST routes moved onto the shared route boundary; BLE001 budget lowered from 129 to 124. Broader HTTP/process boundaries remain open. |
...
- **Exception suppression ratchet:** two URL parsing catches were narrowed from `except Exception`; update POST routes now use `src/bridge/routes/error_boundary.py`; `tools/repo_health/source_suppression_budget.json` now budgets `BLE001` at 124.

Verification evidence for this slice:

- `npm run test:py:extended` passed: `3454 passed, 1 skipped`
...
| 11 | Replace `except Exception` in low-risk files (post_routes_update, adapters, shared) | §4 | ~15 files | Partial | Update POST route slice completed 2026-06-17 |
