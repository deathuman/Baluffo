...
| admin_bridge legacy globals | P0 | Partial | 5-way root injection seam now has explicit coverage. `BridgeServices` holder owns sync config, sync, desktop-update, registry, discovery, and pipeline service state; the pipeline service root mirror has been removed, while sync, desktop-update, registry, and discovery still mirror legacy globals for compatibility. Explicit `global SYNC_CONFIG` refresh declarations are removed. Broader root-injection cleanup remains open. |
...
- **Remaining mutable singleton mirrors** with per-service `RLock`: `_SYNC_SERVICE`, `_REGISTRY_SERVICE`, `_DISCOVERY_SERVICE`, `_DESKTOP_UPDATE_SERVICE` + companion data-dir/path variables; the pipeline service mirror has been removed
...
- Move remaining `_SYNC_SERVICE`, `_REGISTRY_SERVICE`, `_DISCOVERY_SERVICE`, and `_DESKTOP_UPDATE_SERVICE` singleton mirrors into explicit holder objects passed through `BridgeApi` or `RuntimeConfig`; `_PIPELINE_SERVICE` is already removed
