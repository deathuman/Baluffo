...
**69 source instances** of `except Exception`/`suppress(Exception)` remain across **39 source files**:

- 78 total `src` suppression comments, including 70 BLE001 suppressions guarded by budget
- 69 source broad-catch sites remain after excluding one non-broad BLE001 suppression
