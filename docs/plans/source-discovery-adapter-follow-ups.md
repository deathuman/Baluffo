...
| P1c strategy contract normalization | In progress | `PageOutcomeStrategy`, active-audit strategies, browser-recovery assembly, shared recovery request/result payload contracts, and scan-result merge payloads exist. Broader contracts for provenance and remaining adapter-specific result shapes are still future cleanup. |
