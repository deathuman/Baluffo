...
class SocialSourcesExpansionResearcher:
    ...
    def generate_research_report(self) -> str:
        ...
        report.append(f"  • Mastodon: {len(mastodon_expansion['instances'])} instances, {len(mastodon_expansion['hashtags'])} hashtags identified")
        report.append("")
        ...
        report.append("Current subreddits: 6 (gamedev, gameDevClassifieds, gamedevjobs, INAT, gamejobs, indiegaming)")
        report.append("")
        ...
        report.append(f"  Reddit: {analysis['reddit_performance']['estimated_potential']} potential")
        report.append(f"  X (Twitter): {analysis['x_performance']['estimated_potential']} potential")
        report.append(f"  Mastodon: {analysis['mastodon_performance']['estimated_potential']} potential")
        report.append("")
        ...
        report.append("")
        report.append(f"Expected total gain: {analysis['overall_assessment']['total_estimated_gain']}")
        report.append("")
        ...
        report.append(f"  Confidence threshold: {config['reddit']['adjust_confidence_threshold']}% (down from 40%)")
        report.append(f"  Max posts per subreddit: {config['reddit']['adjust_max_posts_per_subreddit']} (up from 50)")
        report.append("")
