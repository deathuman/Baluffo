...
def generate_optimization_recommendations(current_config: Dict[str, Any], performance_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Overall suggestions
    if success_rate < 0.9:
        recommendations["overall_suggestions"].append(f"Overall success rate is low ({success_rate:.1%}). Focus on rate limiting and error handling.")
    if error_rate > 0.1:
        recommendations["overall_suggestions"].append(f"High error rate ({error_rate:.1%}). Review API access and network stability.")
    if avg_duration > 20000:  # 20 seconds
        recommendations["overall_suggestions"].append(f"High average duration ({avg_duration/1000:.1f}s). Consider parallelization or timeout optimization.")
...
def generate_optimization_report(current_config: Dict[str, Any], optimized_config: Dict[str, Any], 
    ...
    if current_config.get("minConfidence") != optimized_config.get("minConfidence"):
        report.append(f"minConfidence: {current_config.get('minConfidence')}% → {optimized_config.get('minConfidence')}%")
    if current_config.get("reddit", {}).get("rateLimitDelay") != optimized_config.get("reddit", {}).get("rateLimitDelay"):
    ...
    if current_config.get("x", {}).get("timeoutSeconds") != optimized_config.get("x", {}).get("timeoutSeconds"):
    ...
    if current_config.get("mastodon", {}).get("retries") != optimized_config.get("mastodon", {}).get("retries"):
...
def main():
    ...
    if not current_config:
        print("❌ No social sources configuration found. Please create data/social-sources-config.json first.")
        return 1
    ...
    if not performance_data:
        print("❌ No performance data found. Please run the social sources pipeline first to gather performance metrics.")
    ...
    optimization_report = generate_optimization_report(current_config, optimized_config, recommendations, performance_data)
