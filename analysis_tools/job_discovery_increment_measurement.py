...
def run_social_pipeline(output_dir: Path, timeout: int = 300) -> Dict[str, Any]:
    ...
    try:
        ...
        if report_path.exists():
            ...
            social_sources = [s for s in report.get("sources", []) if s.get("name", "").startswith(("social_reddit", "social_x", "social_mastodon"))]
...
def generate_increment_report(gains: Dict[str, Any]) -> str:
    ...
    report.append(f"Efficiency Change: {(jobs_per_second_social - jobs_per_second_baseline):+.2f} jobs/s")
    report.append("")
...
def main():
    ...
    print(f"Platform Breakdown: Reddit {gains['social_breakdown']['reddit']['jobs']:,}, X {gains['social_breakdown']['x']['jobs']:,}, Mastodon {gains['social_breakdown']['mastodon']['jobs']:,}")
