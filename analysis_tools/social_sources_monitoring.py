...
def analyze_social_sources_performance(report: dict[str, Any]) -> dict[str, Any]:
    ...
    performance_data = {
        ...
        "success_rates": {"overall": 0, "by_platform": {}},
        "job_discovery": {
            "total_jobs": 0,
            "by_platform": {},
            "by_subreddit": {},
        ...
        "error_analysis": {"total_errors": 0, "by_platform": {}, "error_types": {}},
    }
    ...
    for source in social_sources:
        ...
        performance_data["timing_metrics"]["platform_durations"][platform] = (
            performance_data["timing_metrics"]["platform_durations"].get(platform, 0) + duration
        )

        # Job discovery
        kept_count = int(source.get("keptCount", 0))
        performance_data["job_discovery"]["total_jobs"] += kept_count
        performance_data["job_discovery"]["by_platform"][platform] = (
            performance_data["job_discovery"]["by_platform"].get(platform, 0) + kept_count
        )
        ...
        if status == "error":
            ...
            performance_data["error_analysis"]["by_platform"][platform] = (
                performance_data["error_analysis"]["by_platform"].get(platform, 0) + 1
            )
            error_msg = source.get("error", "")
            if error_msg:
                performance_data["error_analysis"]["error_types"][error_msg] = (
                    performance_data["error_analysis"]["error_types"].get(error_msg, 0) + 1
                )

        # Platform-specific breakdowns
        if platform == "reddit":
            subreddit = name.split(":")[-1] if ":" in name else name
            performance_data["job_discovery"]["by_subreddit"][subreddit] = (
                performance_data["job_discovery"]["by_subreddit"].get(subreddit, 0) + kept_count
            )
        elif platform == "x":
            query = name.split(":")[-1] if ":" in name else name
            performance_data["job_discovery"]["by_query"][query] = (
                performance_data["job_discovery"]["by_query"].get(query, 0) + kept_count
            )
        elif platform == "mastodon":
            instance = name.split(":")[-1] if ":" in name else name
            performance_data["job_discovery"]["by_instance"][instance] = (
                performance_data["job_discovery"]["by_instance"].get(instance, 0) + kept_count
            )
    ...
    for platform, sources in performance_data["by_platform"].items():
        if sources:
            ...
            performance_data["success_rates"]["by_platform"][platform] = len(successful) / len(
                sources
            )
...
def generate_performance_report(performance_data: dict[str, Any]) -> str:
    ...
    for platform in ["reddit", "x", "mastodon"]:
        ...
        if sources:
            ...
            avg_duration = (
                statistics.mean([int(s.get("durationMs", 0)) for s in sources]) if sources else 0
            )
    ...
    report.append(
        f"Average Duration: {performance_data['timing_metrics']['average_duration']:.0f}ms"
    )
    ...
    if performance_data["job_discovery"]["by_subreddit"]:
        ...
        for subreddit, count in sorted(
            performance_data["job_discovery"]["by_subreddit"].items(),
            key=lambda x: x[1],
            reverse=True,
        ):
    ...
    if performance_data["job_discovery"]["by_query"]:
        ...
        for query, count in sorted(
            performance_data["job_discovery"]["by_query"].items(), key=lambda x: x[1], reverse=True
        ):
    ...
    if performance_data["job_discovery"]["by_instance"]:
        ...
        for instance, count in sorted(
            performance_data["job_discovery"]["by_instance"].items(),
            key=lambda x: x[1],
            reverse=True,
        ):
    ...
    if performance_data["error_analysis"]["error_types"]:
        ...
        for error_type, count in sorted(
            performance_data["error_analysis"]["error_types"].items(),
            key=lambda x: x[1],
            reverse=True,
        ):
...
def monitor_error_patterns(error_data: dict[str, Any]):
    ...
    if error_data["error_analysis"]["total_errors"] > 0:
        ...
        print("\nCommon Error Types:")
        for error_type, count in sorted(
            error_data["error_analysis"]["error_types"].items(), key=lambda x: x[1], reverse=True
        )[:5]:
...
def generate_recommendations(performance_data: dict[str, Any]) -> list[str]:
    ...
    if performance_data["success_rates"]["overall"] < 0.9:
        recommendations.append(
            "⚠️  Low overall success rate. Consider adjusting rate limits or timeout settings."
        )

    # Performance recommendations
    if performance_data["timing_metrics"]["average_duration"] > 30000:  # 30 seconds
        recommendations.append(
            "⚠️  High average duration. Consider optimizing fetch strategies or parallelization."
        )

    # Job discovery recommendations
    if performance_data["job_discovery"]["total_jobs"] < 10:
        recommendations.append(
            "⚠️  Low job discovery rate. Consider adding more subreddits/queries or adjusting confidence thresholds."
        )
    ...
    for platform in ["reddit", "x", "mastodon"]:
        ...
        if success_rate < 0.8:
            recommendations.append(
                f"⚠️  {platform.upper()} success rate below 80%. Review configuration and API access."
            )

    # Positive recommendations
    if not recommendations:
        recommendations.append(
            "✅ All metrics look good! Consider expanding coverage with additional sources."
        )
