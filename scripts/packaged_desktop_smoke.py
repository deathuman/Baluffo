...
def fetch_startup_metrics(bridge_base_url: str, limit: int = 400) -> List[Dict[str, Any]]:
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            metrics_rows = fetch_startup_metrics(bridge_base_url, limit=200)
...
def capture_runtime_snapshot(bridge_base_url: str, artifacts_dir: Path) -> Dict[str, str]:
    ...
    metrics_payload = fetch_json(f"{bridge_base_url}/desktop-local-data/startup-metrics?limit=400")
...
def wait_for_runtime_events(bridge_base_url: str, required_events: List[str] | tuple[str, ...], timeout_s: float) -> List[Dict[str, Any]]:
    ...
    while time.monotonic() < deadline:
        rows = fetch_startup_metrics(bridge_base_url, limit=400)
