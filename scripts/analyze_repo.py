...
def analyze_pillar_build_system(root: Path) -> dict[str, Any]:
    ...
    if check_file_exists(
        root, "Makefile", "package.json", "pyproject.toml", "setup.py", "setup.cfg"
    ):
    ...
    if check_file_exists(
        root, "package-lock.json", "requirements.lock", "Pipfile.lock", "poetry.lock", "yarn.lock"
    ):
    ...
    if check_file_exists(
        root, "package-lock.json", "requirements.lock", "Pipfile.lock", "poetry.lock"
    ):
...
def analyze_pillar_testing(root: Path) -> dict[str, Any]:
    ...
    if check_directory_exists(root, ".github/workflows", ".gitlab-ci.yml", "azure-pipelines.yml"):
...
def analyze_pillar_dev_environment(root: Path) -> dict[str, Any]:
    ...
    if check_file_exists(
        root, "Makefile", "package.json", "pyproject.toml", "setup.py", "requirements.txt"
    ):
...
def analyze_pillar_debugging(root: Path) -> dict[str, Any]:
    ...
    if check_file_exists(
        root, "docs/TROUBLESHOOTING.md", "TROUBLESHOOTING.md", "docs/troubleshooting.md"
    ):
...
def analyze_pillar_versioning(root: Path) -> dict[str, Any]:
    ...
    if check_file_exists(root, "pyproject.toml", "package.json", "__version__.py", "version.py"):
...
def analyze_pillar_security(root: Path) -> dict[str, Any]:
    ...
    if check_file_exists(root, "SECURITY.md", "security.md", ".github/SECURITY.md"):
        results["met"].append("8.1")
    else:
        results["unmet"].append("8.1")

    # 8.2 Dependencies are scanned (check for safety config or dependabot)
    if check_file_exists(root, ".github/dependabot.yml", "safety.txt", ".snyk", "pyproject.toml"):
...
def analyze_pillar_onboarding(root: Path) -> dict[str, Any]:
    ...
    if check_directory_exists(root, "examples", "sample", "demo") or check_file_exists(
        root, "example", "samples"
    ):
...
def analyze_repository(root_path: str = ".") -> dict[str, Any]:
    ...
    analysis = {"repository": str(root), "pillars": {}}
