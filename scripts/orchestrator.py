...
def rotate_history(max_runs: int = 5):
    ...
    try:
        run_items = sorted([d for d in RUNS.iterdir() if d.is_dir()], key=lambda x: x.stat().st_mtime)
...
def build(args: argparse.Namespace, run_dir: Path | None = None, is_verify: bool = False) -> tuple[bool, Path | None]:
    ...
    ok, log = run_proc([sys.executable, "scripts/build_ship_bundle.py", "--output-dir", str(ship_out)], "ShipBundle")
    if not ok:
    ...
    ok, log = run_proc([sys.executable, "scripts/build_portable_exe.py", "--output-dir", str(exe_out), "--skip-zip"], "PortableEXE")
    if not ok:
...
def verify(args: argparse.Namespace):
    ...
    if not exe_path.exists():
        ...
        ok_smoke, log_smoke = run_proc([
            sys.executable, "src/packaged_desktop_smoke.py", 
            "--exe-path", str(exe_path),
            "--artifacts-dir", str(smoke_dir),
            "--report-path", str(smoke_dir / "report.json")
        ], "SmokeTest", allow_stream=True)
...
def main():
    ...
    build_parser.add_argument("--force", action="store_true", help="Force build even if hash matches")
    
    verify_parser = subparsers.add_parser("verify", help="Full verification (Build + Test)")
    verify_parser.add_argument("--force", action="store_true", help="Force build even if hash matches")
