...
def test_pipeline_schedule_no_prior_run_starts_once_when_enabled(tmp_path: Path) -> None:
    harness = _Harness(tmp_path)

    result = harness.service.update_config({"enabled": True, "intervalHours": "24"})

    assert result["status"]["lastTriggerRunId"] == "pipeline_scheduled"
    assert harness.started_payloads == [
        {"trigger": "schedule", "automatic": True, "jobsPageLoadedCount": 0}
    ]
