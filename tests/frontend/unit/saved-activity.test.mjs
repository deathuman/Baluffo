...
test("saved timeline helpers classify activity entry types", () => {
  ...
  assert.equal(timelineTypeForEntry({ type: "outcome_changed" }), "phase");
...
test("saved timeline helpers filter entries by scope", () => {
  const entries = [
    ...
    { type: "outcome_changed", jobKey: "job_3", createdAt: "2026-03-08T13:00:00.000Z" }
  ];

  assert.equal(filterActivityEntriesForScope(entries, "all", "").length, 4);
  assert.equal(filterActivityEntriesForScope(entries, "selected", "job_1").length, 2);
  assert.equal(filterActivityEntriesForScope(entries, "phase", "").length, 2);
