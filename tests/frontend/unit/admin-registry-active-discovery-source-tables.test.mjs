...
test("admin registry loads compact source tables during pipeline discovery", async () => {
  const calls = [];
      ...
      getBridge: async path => {
        ...
        if (String(path).startsWith("/registry/sources")) {
          return {
            ok: true,
            sources: {
              pending: [{ name: "Pending Discovery Studio", sourceId: "p1", url: "https://pending.example" }],
              active: [{ name: "Active Discovery Studio", sourceId: "a1", url: "https://active.example" }],
              rejected: []
            },
...
test("admin registry renders delayed placeholders when discovery source table load times out", async () => {
  const logs = [];
  const fixture = createRegistryControllerFixture({
    state: {
      adminBusyState: {
