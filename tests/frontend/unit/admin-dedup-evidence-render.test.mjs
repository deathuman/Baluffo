...
test("admin render: fetcher metrics render read-only dedup evidence", () => {
  ...
  assert.match(metricsEl.innerHTML, /2 blocking issues/i);
  assert.match(metricsEl.innerHTML, /Provider\/static disagreements/i);
  assert.match(metricsEl.innerHTML, /Current-run high-risk review queue/i);
  assert.match(metricsEl.innerHTML, /Why blocked/i);
  assert.match(metricsEl.innerHTML, /Next action/i);
  assert.match(metricsEl.innerHTML, /Gate metrics/i);
  ...
  assert.doesNotMatch(metricsEl.innerHTML, /non-provider URL \d/i);
  ...
  assert.match(metricsEl.innerHTML, /<strong>Suspected cause<\/strong> listing page bundle/);
  assert.match(metricsEl.innerHTML, /<strong>Identity shape<\/strong> shared listing or category url/);
  assert.match(metricsEl.innerHTML, /<strong>Identity quality<\/strong> shared listing url weak/);
  assert.match(metricsEl.innerHTML, /<strong>Provenance<\/strong> google sheets row identity/);
  assert.match(metricsEl.innerHTML, /<strong>Sheets bundle shape<\/strong> role category bucket/);
  assert.match(metricsEl.innerHTML, /<strong>Sheets role-bucket audit<\/strong> listing or search url bucket/);
  assert.match(metricsEl.innerHTML, /<strong>Sheets bucket intent<\/strong> listing or search bucket/);
  assert.match(metricsEl.innerHTML, /<strong>Sheets weak grouping audit<\/strong> role bucket listing grouping/);
  assert.match(metricsEl.innerHTML, /<strong>Caveats<\/strong> shared url looks like listing or category/);
  assert.match(metricsEl.innerHTML, /<strong>Identity evidence<\/strong> quality:shared listing url weak/);
  assert.match(metricsEl.innerHTML, /<strong>Provenance evidence<\/strong> provenance:google sheets row identity/);
  assert.match(metricsEl.innerHTML, /<strong>Sheets evidence<\/strong> shape:role category bucket/);
  assert.match(metricsEl.innerHTML, /<strong>Sheets audit evidence<\/strong> audit:listing or search url bucket/);
  assert.match(metricsEl.innerHTML, /<strong>Sheets intent evidence<\/strong> intent:listing or search bucket/);
  assert.match(metricsEl.innerHTML, /<strong>Sheets weak audit evidence<\/strong> audit:role bucket listing grouping/);
  assert.match(metricsEl.innerHTML, /<strong>Cause evidence<\/strong> cause:listing page bundle/);
  assert.match(metricsEl.innerHTML, /<strong>Sources<\/strong> kforce-a \| kforce-b/);
