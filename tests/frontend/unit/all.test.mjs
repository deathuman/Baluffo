// Auto-generated by scripts/sync_frontend_unit_manifest.mjs.
// Do not hand-edit this file; run `npm run sync:test-manifest` instead.
import "./actions.test.mjs";
import "./admin-auth-controller.test.mjs";
import "./admin-busy-state.test.mjs";
import "./admin-config.test.mjs";
import "./admin-discovery-controller.test.mjs";
import "./admin-domain.test.mjs";
import "./admin-fetcher-controller.test.mjs";
import "./admin-live-task-restore.test.mjs";
import "./admin-live-task.test.mjs";
import "./admin-ops-controller.test.mjs";
import "./admin-progress-ui.test.mjs";
import "./admin-registry-controller.test.mjs";
import "./admin-render-log.test.mjs";
import "./admin-render.test.mjs";
import "./admin-sync-controller.test.mjs";
import "./desktop-local-data-navigation.test.mjs";
import "./jobs-admin-bridge-state.test.mjs";
import "./jobs-data-source.test.mjs";
import "./jobs-desktop-update.test.mjs";
import "./jobs-domain.test.mjs";
import "./jobs-feed-startup.test.mjs";
import "./jobs-html.test.mjs";
import "./jobs-parsing-utils.test.mjs";
import "./jobs-pipeline-controller.test.mjs";
import "./jobs-pipeline.test.mjs";
import "./jobs-render.test.mjs";
import "./jobs-runtime-events.test.mjs";
import "./jobs-runtime-feed-controller.test.mjs";
import "./jobs-runtime-jobs-list-events.test.mjs";
import "./jobs-runtime-list-view.test.mjs";
import "./jobs-runtime-query.test.mjs";
import "./jobs-runtime-startup-preview.test.mjs";
import "./jobs-runtime-state.test.mjs";
import "./jobs-runtime-url-persistence.test.mjs";
import "./jobs-source-metadata.test.mjs";
import "./jobs-state.test.mjs";
import "./live-task-observability.test.mjs";
import "./local-data-backup-domain.test.mjs";
import "./local-data-backup-import-export.test.mjs";
import "./local-data-domains.test.mjs";
import "./local-data-runtime-contract.test.mjs";
import "./local-data-services.test.mjs";
import "./manifest-contract.test.mjs";
import "./mcp-playwright-server.test.mjs";
import "./playwright-smoke-runtime.test.mjs";
import "./release-notes-dialog.test.mjs";
import "./saved-admin-bridge-state.test.mjs";
import "./saved-data-source.test.mjs";
import "./saved-domain.test.mjs";
import "./saved-notes-editing.test.mjs";
import "./saved-phase-time.test.mjs";
import "./saved-render-anchor.test.mjs";
import "./saved-render.test.mjs";
import "./saved-runtime-controllers.test.mjs";
import "./saved-timeline.test.mjs";
import "./startup-metrics-effects.test.mjs";
import "./structure-cleanup.test.mjs";
import "./task-progress.test.mjs";
