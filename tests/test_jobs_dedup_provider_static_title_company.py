...
def test_dedup_evidence_warns_on_current_same_city_location_variant() -> None:
    evidence = build_dedup_evidence(
        {"mergedCount": 1},
        [
            _provider_static_row(
                sourceBundle=[
                    {
                        "source": "greenhouse:slug:studio-one",
                        "sourceJobId": "greenhouse:studio-one:123",
                        "jobLink": "https://job-boards.greenhouse.io/studioone/jobs/123",
                        "adapter": "greenhouse",
                    },
                    {
                        "source": "static_source::static:listing_url:https://studio.example/careers",
    ...
    assert evidence["providerStaticTitleCompanyCollisionCounts"]["total"] == 0
    assert evidence["providerStaticDisagreementExamples"] == []
    assert evidence["providerStaticDisagreementGateCounts"]["blocked"] == 0


def test_dedup_evidence_warns_on_current_role_token_location_pollution() -> None:
    evidence = build_dedup_evidence(
        {"mergedCount": 1},
        [
            _provider_static_row(
                ...
                sourceBundle=[
                    {
                        "source": "smartrecruiters:EpochGames",
                        "sourceJobId": "smartrecruiters:EpochGames:744000091323145",
                        "jobLink": "https://jobs.smartrecruiters.com/EpochGames/744000091323145",
                        "adapter": "smartrecruiters",
                    },
                    {
                        "source": "static_source::static:listing_url:https://careers.smartrecruiters.com/epochgames",
                        "sourceJobId": "static-1",
                        "jobLink": "https://jobs.smartrecruiters.com/EpochGames/744000091323145-concept-artist-illustrator",
                        "adapter": "static",
                    },
                ],
                locations=[
    ...
    assert "single_effective_location_variant" in row["disagreementClassificationEvidence"]
    assert evidence["providerStaticDisagreementGateCounts"]["blocked"] == 0
...
def test_dedup_evidence_audits_carried_location_pollution_from_title_overlap() -> None:
    ...
    assert "garbage_category:role_category" in collision["carriedLocationPollutionEvidence"]
...
def test_dedup_evidence_audits_carried_location_pollution_from_repeated_company_token() -> None:
    ...
    assert all(
        "garbage_category:role_category" in row["carriedLocationPollutionEvidence"] for row in rows
    )
    assert evidence["providerStaticTitleCompanyCollisionAuditCounts"] == {
