...
def test_trigger_source_check_static_fallback_uses_generic_scrape(admin_bridge_ops_root):
...
def test_trigger_source_check_static_fallback_returns_failure_when_no_jobs(admin_bridge_ops_root):
    added = admin_bridge.add_manual_source("https://milestone.it/careers/")
    source_id = str(added.get("sourceId") or "")
    assert source_id
    ...
    try:
        ...
        assert "no job postings found" in str(result.get("error") or "").lower()
    finally:
        admin_bridge.discovery.fetch_text_with_retry = original_fetch
...
def test_trigger_source_check_static_fallback_classifies_rendered_404_page(admin_bridge_ops_root):
    ...
    try:
        ...
        assert result["started"]
        assert not result["ok"]
        assert str(result.get("errorCode") or "") == "not_found"
        assert bool(result.get("suggestedUrls"))
    finally:
        admin_bridge.discovery.fetch_text_with_retry = original_fetch
...
def test_trigger_source_check_static_fallback_attempts_browser_on_403(admin_bridge_ops_root):
    ...
    try:
        ...
        assert bool(result.get("weakSignal"))
        assert int(result["jobsFound"]) == 1
        assert bool(result.get("browserFallbackAttempted"))
...
def test_trigger_source_check_static_parses_embedded_job_filter_payload(admin_bridge_ops_root):
    ...
    try:
        ...
        assert result["started"]
        assert result["ok"]
        assert not bool(result.get("weakSignal"))
        assert int(result["jobsFound"]) == 3
        assert not bool(result.get("browserFallbackAttempted"))
...
def test_trigger_source_check_static_fallback_reports_unavailable_browser_fallback(admin_bridge_ops_root):
    ...
    try:
        ...
        assert result["started"]
        assert not result["ok"]
        assert str(result.get("errorCode") or "") == "browser_fallback_unavailable"
        assert bool(result.get("browserFallbackAttempted"))
        assert not bool(result.get("browserFallbackUsed"))
    finally:
        admin_bridge.discovery.fetch_text_with_retry = original_fetch
...
def test_trigger_source_check_static_fallback_returns_404_hints(admin_bridge_ops_root):
    ...
    try:
        ...
        assert "https://careers.king.com" in suggested
    finally:
        admin_bridge.discovery.fetch_text_with_retry = original_fetch
