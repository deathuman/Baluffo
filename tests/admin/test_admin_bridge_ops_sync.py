...
def test_update_saved_sync_settings_persists_local_enablement_only(admin_bridge_ops_root):
...
def test_sync_pull_updates_local_registry_counts(admin_bridge_ops_root):
...
def test_start_sync_task_creates_started_history_row(admin_bridge_ops_root):
    admin_bridge.update_saved_sync_settings({"enabled": True})
...
def test_sync_history_discards_stale_sync_started_row_without_run_id(
...
def test_sync_worker_failure_writes_error_row(admin_bridge_ops_root):
    started_at = admin_bridge.now_iso()
    admin_bridge.append_run_history(
        {
            "id": "sync_test_err",
...
def test_wait_for_report_completion_ignores_stale_flag_until_report_finishes(admin_bridge_ops_root):
    started_at = admin_bridge.now_iso()
    ...
    with (
        ...
        result = admin_bridge._wait_for_report_completion(  # noqa: SLF001
            report_path=admin_bridge_ops_root / "source-discovery-report.json",
...
def test_wait_for_report_completion_can_fail_fast_when_stale_guard_enabled(admin_bridge_ops_root):
    started_at = admin_bridge.now_iso()
    ...
    with (
        ...
        with pytest.raises(RuntimeError):
            admin_bridge._wait_for_report_completion(  # noqa: SLF001
                report_path=admin_bridge_ops_root / "jobs-fetch-report.json",
                started_at=started_at,
                timeout_s=10.0,
